...
for phi(z)=a(1+z)+e psi(z). The affine gain disappears from the numerator. For 0<=e<=1, a>=2, centered Gaussian Z of standard deviation sigma>=1, and normalized shape bounds ||psi||_infinity, ||psi' ||_infinity<=1, the numerator is at most e^2, while the denominator E phi(Z)^2 is at least (a-e)^2 sigma^2, by projection onto Z and Gaussian integration by parts. The relative regression fraction is therefore at most 4e^2/a^2 for a>=2. A nonzero hidden velocity or changing kernel also does not by itself exclude a deep affine explanation.
...
Throughout the depth comparisons, beta is a fixed positive number. A true residual layer is
