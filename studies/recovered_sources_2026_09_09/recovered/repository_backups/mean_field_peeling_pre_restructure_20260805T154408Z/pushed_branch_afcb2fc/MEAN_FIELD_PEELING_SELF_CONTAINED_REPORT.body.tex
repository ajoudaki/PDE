...
\addcontentsline{toc}{section}{Executive orientation}
...
\addcontentsline{toc}{subsection}{What is established, and what is not}


Four levels of claim are used throughout.

\begin{enumerate}
\item \textbf{Exact finite-width identity.} This follows from ordinary differentiation,
   conditioning, Gaussian integration by parts, or Taylor expansion. No width
   limit has been taken.
\item \textbf{Audited mean-field result.} The displayed limit follows from the peel
...
\item \textbf{Formal mean-field closure.} The finite differentiated state and its local
...
\item \textbf{Open theorem program.} The broad assertion that every admissible
...
exact only \textbf{conditionally and one layer at a time}. This conditional law is
enough for peeling.

\subsection*{Headline computational result}
\addcontentsline{toc}{subsection}{Headline computational result}
...
\section*{1. Baseline network and the mean-field state}
\addcontentsline{toc}{section}{1. Baseline network and the mean-field state}


\subsection*{1.1 General fully connected network}
\addcontentsline{toc}{subsection}{1.1 General fully connected network}
...
\begin{itemize}
\item every width and limiting width ratio;
\item raw and effective parameter coordinates;
\item weight and bias variances;
\item output normalization;
\item optimizer or \(\mu\)P metric factors;
\item normalization of the observable;
\item desired order in width;
\item whether the target is an expectation, deterministic limit, or fluctuation.
\end{itemize}
...
\subsection*{1.2 Exact conditional Gaussianity}
\addcontentsline{toc}{subsection}{1.2 Exact conditional Gaussianity}
...
\subsection*{1.3 Deterministic forward Gram recursion}
\addcontentsline{toc}{subsection}{1.3 Deterministic forward Gram recursion}
...
\section*{2. What may be peeled}
\addcontentsline{toc}{section}{2. What may be peeled}


\subsection*{2.1 Groups and explicit atoms}
\addcontentsline{toc}{subsection}{2.1 Groups and explicit atoms}


Group \(\ell\) contains:

\begin{itemize}
\item layer-\(\ell\) weights and biases;
\item coordinates \(z_i^\ell(a)\);
\item scalar functions of those coordinates, including
  \(\phi,\phi',\phi'',\ldots\).
\end{itemize}
...
\subsection*{2.2 Scalar contraction normal form}
\addcontentsline{toc}{subsection}{2.2 Scalar contraction normal form}
...
\begin{itemize}
\item neuron indices and their layers;
\item fixed batch labels;
\item exact equality and inequality constraints;
\item derivative order of each scalar function;
\item all explicit powers of width;
\item open lower-layer indices created by the current peel.
\end{itemize}
...
\section*{3. The exact Gaussian calculus}
\addcontentsline{toc}{section}{3. The exact Gaussian calculus}


\subsection*{3.1 One-weight Stein identity}
\addcontentsline{toc}{subsection}{3.1 One-weight Stein identity}
...
\subsection*{3.2 Two-weight identity}
\addcontentsline{toc}{subsection}{3.2 Two-weight identity}
...
\subsection*{3.3 General partial-matching identity}
\addcontentsline{toc}{subsection}{3.3 General partial-matching identity}
...
\begin{itemize}
\item Each matched pair is a weight-weight Wick contraction.
\item Each unmatched weight attaches to one preactivation by Stein's identity.
\item Each attachment differentiates the top nonlinear factor.
\item Each attachment inserts a lower-layer activation through its covariance.
\end{itemize}
...
\subsection*{3.4 Equality partitions}
\addcontentsline{toc}{subsection}{3.4 Equality partitions}
...
\subsection*{3.5 Global width counting}
\addcontentsline{toc}{subsection}{3.5 Global width counting}


The width degree of a diagram includes:

\begin{itemize}
\item one positive power for every free neuron-index block;
\item negative powers from weight covariances;
\item negative powers from weight-preactivation attachments;
\item observable normalization;
\item raw-to-effective parameter multipliers;
\item optimizer or \(\mu\)P metric factors;
\item lower-layer sums created by Stein attachments.
\end{itemize}
...
\subsection*{3.6 Replacing random Grams}
\addcontentsline{toc}{subsection}{3.6 Replacing random Grams}
...
\section*{4. The peeling algorithm}
\addcontentsline{toc}{section}{4. The peeling algorithm}


Given an admissible scalar contraction, perform the following pass.

\begin{enumerate}
\item Fix the scaling ledger and the desired asymptotic order.
\item Expand derivatives only far enough to expose the highest active group.
\item Scalarize every contraction involving that group.
\item Record exact width factors and all lower-layer boundary indices.
\item Condition on all lower groups.
\item Split top-row indices into compatible equality partitions.
\item Apply the generalized Wick-Stein identity until no explicit top-group
   Gaussian parameter remains.
\item Expand all derivatives created on nonlinear factors.
\item Substitute exact conditional covariances.
\item Resolve Kronecker deltas and update the lower-layer boundary state.
\item Perform global width counting, including all downstream sums.
\item Convert surviving top-row functions to Gaussian row moments.
\item Replace the empirical covariance by its deterministic limit, or retain
    the covariance fluctuation required by the target.
\item Merge algebraically identical lower-layer states.
\item Repeat on the next lower group.
\item At layer zero, evaluate the remaining deterministic input and label
    contractions.
\end{enumerate}
...
\subsection*{4.1 Recursive-state form}
\addcontentsline{toc}{subsection}{4.1 Recursive-state form}
...
\section*{5. Worked execution I: the backward kernel}
\addcontentsline{toc}{section}{5. Worked execution I: the backward kernel}


\subsection*{5.1 Fixed three-hidden-layer network}
\addcontentsline{toc}{subsection}{5.1 Fixed three-hidden-layer network}
...
\subsection*{5.2 Normalized backward variables}
\addcontentsline{toc}{subsection}{5.2 Normalized backward variables}
...
\subsection*{5.3 Peel the readout and group 3}
\addcontentsline{toc}{subsection}{5.3 Peel the readout and group 3}
...
\subsection*{5.4 Full group-3 scalarization for the layer-2 kernel}
\addcontentsline{toc}{subsection}{5.4 Full group-3 scalarization for the layer-2 kernel}
...
\subsection*{5.5 Continue through group 2 and group 1}
\addcontentsline{toc}{subsection}{5.5 Continue through group 2 and group 1}
...
\begin{enumerate}
\item peel the readout, forcing \(r=s\);
\item condition on groups 1 and 2 and peel group 3;
\item retain both the direct \(W^3\)-pairing branch and every Stein branch;
\item resolve the induced equalities among \(p,q,r\);
\item pass any group-2 activations created by group-3 Stein derivatives downward;
\item peel group 2;
\item finally average the remaining group-1 gate product.
\end{enumerate}
...
\subsection*{5.6 Expectation is not concentration}
\addcontentsline{toc}{subsection}{5.6 Expectation is not concentration}
...
\section*{6. Worked execution II: one muP gradient step}
\addcontentsline{toc}{section}{6. Worked execution II: one muP gradient step}


\subsection*{6.1 Parameterization and update}
\addcontentsline{toc}{subsection}{6.1 Parameterization and update}
...
\subsection*{6.2 Exact optimizer-metric tangent kernel}
\addcontentsline{toc}{subsection}{6.2 Exact optimizer-metric tangent kernel}
...
\subsection*{6.3 Linear change in output and loss}
\addcontentsline{toc}{subsection}{6.3 Linear change in output and loss}
...
\subsection*{6.4 Exact hidden-Gram linear coefficient}
\addcontentsline{toc}{subsection}{6.4 Exact hidden-Gram linear coefficient}
...
\subsection*{6.5 Exact quadratic Gram expansion}
\addcontentsline{toc}{subsection}{6.5 Exact quadratic Gram expansion}
...
\subsection*{6.6 Backward and tangent states used by the quadratic peel}
\addcontentsline{toc}{subsection}{6.6 Backward and tangent states used by the quadratic peel}
...
\subsection*{6.7 Why the off-diagonal branch survives}
\addcontentsline{toc}{subsection}{6.7 Why the off-diagonal branch survives}
...
\subsection*{6.8 Closed one-step Gaussian recursions}
\addcontentsline{toc}{subsection}{6.8 Closed one-step Gaussian recursions}
...
\subsection*{6.9 Fully explicit first-layer answer}
\addcontentsline{toc}{subsection}{6.9 Fully explicit first-layer answer}
...
\subsection*{6.10 Deep-linear audit}
\addcontentsline{toc}{subsection}{6.10 Deep-linear audit}
...
\section*{7. Worked execution III: two Euler steps}
\addcontentsline{toc}{section}{7. Worked execution III: two Euler steps}


\subsection*{7.1 Exact Taylor identity}
\addcontentsline{toc}{subsection}{7.1 Exact Taylor identity}
...
\subsection*{7.2 Isolating the gradient correction}
\addcontentsline{toc}{subsection}{7.2 Isolating the gradient correction}
...
\subsection*{7.3 Exact readout peel}
\addcontentsline{toc}{subsection}{7.3 Exact readout peel}
...
\begin{enumerate}
\item No hidden weight has been declared independent of the preactivation it
creates.
\end{enumerate}


\subsection*{7.4 Differentiated-state program}
\addcontentsline{toc}{subsection}{7.4 Differentiated-state program}
...
\subsection*{7.5 Multichannel Wick-Stein rule}
\addcontentsline{toc}{subsection}{7.5 Multichannel Wick-Stein rule}
...
\subsection*{7.6 Status of the nonlinear two-step closure}
\addcontentsline{toc}{subsection}{7.6 Status of the nonlinear two-step closure}


A reusable formal program is now clear:

\begin{enumerate}
\item compute the forward Gaussian state;
\item peel the ordinary backward state;
\item peel the first tangent \(u,g\);
\item start \(\dot\delta\) at the readout and peel the differentiated backward
   pass from group 3 to group 1;
\item peel the correction-forward states \(\bar z,\bar h\) from group 1 to
   group 3;
\item evaluate the final Gram contraction.
\end{enumerate}
...
is a \textbf{formal general nonlinear closure}, not yet a fully proved general
...
\subsection*{7.7 Deep-linear two-step audit}
\addcontentsline{toc}{subsection}{7.7 Deep-linear two-step audit}
...
\section*{8. Feature-learning interpretation}
\addcontentsline{toc}{section}{8. Feature-learning interpretation}


\subsection*{8.1 Initialization jets}
\addcontentsline{toc}{subsection}{8.1 Initialization jets}
...
\subsection*{8.2 What initialization jets do not prove}
\addcontentsline{toc}{subsection}{8.2 What initialization jets do not prove}


Fixed-order coefficients do not by themselves establish:

\begin{itemize}
\item interchange of time differentiation and the width limit;
\item concentration of every coefficient;
\item a nonzero Taylor radius uniform in width;
\item recovery of finite-time training from the entire local jet;
\item validity for a number of discrete steps growing as \(1/\eta\).
\end{itemize}


Those are analytic questions beyond the algebraic peeling calculus.

\section*{9. A precise theorem target}
\addcontentsline{toc}{section}{9. A precise theorem target}
...
\subsection*{9.1 The plausible novelty}
\addcontentsline{toc}{subsection}{9.1 The plausible novelty}
...
\begin{itemize}
\item accept a contracted fixed-order derivative observable;
\item compile it into a finite equality-partition and boundary-signature state;
\item eliminate each layer by explicit conditional Wick-Stein rules;
\item output a human-readable Gaussian normal form;
\item expose the exact leading diagrams and every Onsager correction.
\end{itemize}
...
\section*{10. Amendments learned from the executions}
\addcontentsline{toc}{section}{10. Amendments learned from the executions}
...
\begin{enumerate}
\item \textbf{Replace global Gaussianity by layerwise conditional Gaussianity.}
   Across layers, the unconditional law is generally a Gaussian mixture.
\item \textbf{Do not replace weight-activation dependence by pseudo-independence.}
...
\item \textbf{Peel the readout before hidden layers.} It controls parity, width, and
   the correct boundary state.
\item \textbf{Keep Stein-created lower-group factors.} A factor created in the
   current peel belongs to the layer where its activation was produced and
   must be delegated downward.
\item \textbf{Separate equality patterns before averaging.} Diagonal and
   row-distinct families can have different limits.
\item \textbf{Count widths globally.} Current-layer covariance powers, lower-layer
   free sums, optimizer factors, and observable normalization all enter the
   same degree.
\item \textbf{Do not infer irrelevance from zero one-copy mean.} Row-distinct fields
   can survive in a square and become leading Gaussian fluctuation channels.
\item **Perform exact conditional integration before deterministic Gram
   replacement.** Distinguished rows require leave-one-out control.
\item \textbf{Separate expectation from concentration.} A deterministic replacement
   inside later products needs a multi-copy proof.
\item \textbf{Register all channels sharing a matrix.} At higher training order,
    fresh fields are generally correlated. Their entire covariance block is
    part of the state.
\item \textbf{Do not differentiate a limiting cancellation.} A sequence whose mean
    tends to zero can have a directional derivative with a nonzero limit.
\item \textbf{Allow Gaussian dimension \(qB\), not always just \(B\).} Each primitive
    row moment uses a \(B\)-vector, but higher-order states can require several
    jointly generated vectors.
\item \textbf{Treat \(O(L)\) complexity as a theorem conclusion.} It requires a
    depth-uniform state bound, not merely a layerwise recipe.
\item \textbf{State regularity caveats.} ReLU needs weak derivatives, smoothing, or
    Gaussian boundary terms. Biases can have order-one Stein covariances and
    cannot be ignored.
\end{enumerate}


\section*{11. Proof obligations for a complete theorem}
\addcontentsline{toc}{section}{11. Proof obligations for a complete theorem}


A full proof should be divided into modules.

\subsection*{11.1 Algebraic compilation}
\addcontentsline{toc}{subsection}{11.1 Algebraic compilation}
...
\subsection*{11.2 Exact layer elimination}
\addcontentsline{toc}{subsection}{11.2 Exact layer elimination}
...
\subsection*{11.3 Uniform moments and leave-one-out}
\addcontentsline{toc}{subsection}{11.3 Uniform moments and leave-one-out}
...
\subsection*{11.4 Joint conditional CLT}
\addcontentsline{toc}{subsection}{11.4 Joint conditional CLT}
...
\subsection*{11.5 Global diagram degree}
\addcontentsline{toc}{subsection}{11.5 Global diagram degree}
...
\subsection*{11.6 Concentration}
\addcontentsline{toc}{subsection}{11.6 Concentration}
...
\subsection*{11.7 Finite-state closure}
\addcontentsline{toc}{subsection}{11.7 Finite-state closure}
...
\section*{12. Practical worksheet}
\addcontentsline{toc}{section}{12. Practical worksheet}


For a new problem, the following worksheet prevents most mistakes.

\subsection*{12.1 Specify}
\addcontentsline{toc}{subsection}{12.1 Specify}


\begin{itemize}
\item What is the raw parameter?
\item What effective tensor enters the forward pass?
\item What are the initialization variances?
\item What optimizer factor multiplies each raw gradient?
\item What scalar observable is being computed?
\item Is the target its expectation, deterministic limit, or fluctuation?
\item What width order is required?
\end{itemize}


\subsection*{12.2 Compile}
\addcontentsline{toc}{subsection}{12.2 Compile}


\begin{itemize}
\item Expand only the highest active group.
\item Write every matrix product with explicit indices.
\item Record all width powers.
\item Record every equality and inequality constraint.
\item Mark the group of every remaining factor.
\end{itemize}


\subsection*{12.3 Peel}
\addcontentsline{toc}{subsection}{12.3 Peel}


\begin{itemize}
\item Condition on lower groups.
\item Split exact equality partitions.
\item Apply every Wick and Stein branch.
\item Differentiate the nonlinear factor explicitly.
\item Insert the exact conditional covariance.
\item Carry newly created lower activations downward.
\item Count the complete diagram before discarding it.
\end{itemize}


\subsection*{12.4 Close}
\addcontentsline{toc}{subsection}{12.4 Close}


\begin{itemize}
\item Convert row averages into Gaussian moments.
\item Justify empirical-to-deterministic covariance replacement.
\item Register fresh Gaussian channels and all cross-covariances.
\item Repeat for the next group.
\item If using the result inside a product, prove concentration separately.
\end{itemize}


\subsection*{12.5 Audit}
\addcontentsline{toc}{subsection}{12.5 Audit}


\begin{itemize}
\item Check parity in centered readouts.
\item Check a linear activation specialization.
\item Check a one-sample or diagonal batch case.
\item Compare width degree against a direct tensor calculation.
\item Verify that a zero limiting term was not differentiated away.
\end{itemize}


\section*{13. Consolidated formula sheet}
\addcontentsline{toc}{section}{13. Consolidated formula sheet}
...
\section*{14. Source and status ledger}
\addcontentsline{toc}{section}{14. Source and status ledger}
...
\begin{itemize}
\item the living mean-field peeling program, which contains the original raw
  proposal, the corrected conditional-Gaussian formulation, and the complete
  backward-kernel audit;
\item the consolidated \(\mu\)P training case study, which contains the
  initialization, one-step, and two-step executions in one convention;
\item the theorem-framing and Tensor Programs contrast report, which sharpens the
...
\section*{15. Closing perspective}
\addcontentsline{toc}{section}{15. Closing perspective}
