...
struct TypePairingCounter {
    int P, target;
    std::unordered_map<std::string,i128> local_memo;
    std::unordered_map<std::string,i128>*memo=&local_memo;

    static void refine(const std::vector<int>&a,const std::vector<int>&h,
                       const std::vector<int>&m,const std::vector<int>&all,
                       std::vector<std::vector<int>>&cells) {
        int rows=a.size(),cols=h.size(),n=rows+cols;
        while(true) {
            bool changed=false;std::vector<std::vector<int>>next;
            for(const auto&cell:cells) {
                std::map<std::vector<int>,std::vector<int>>groups;
                for(int v:cell) {
                    std::vector<int> sig;
                    for(const auto&target_cell:cells) {
                        int occupied=0,remaining=0,raw=0;
                        for(int w:target_cell) {
                            int q=-1;
                            if(v<rows && w>=rows)q=v*cols+(w-rows);
                            else if(v>=rows && w<rows)q=w*cols+(v-rows);
                            if(q>=0 && all[q]){++occupied;remaining+=m[q];raw+=all[q];}
                        }
                        sig.push_back(occupied);sig.push_back(remaining);sig.push_back(raw);
                    }
                    groups[sig].push_back(v);
                }
                if(groups.size()>1)changed=true;
                for(auto&[sig,vertices]:groups)next.push_back(std::move(vertices));
            }
            cells.swap(next);if(!changed)return;
        }
    }
    static std::string discrete_code(
            const std::vector<int>&a,const std::vector<int>&h,
            const std::vector<int>&m,const std::vector<int>&all,
            const std::vector<std::vector<int>>&cells,int P) {
        int nr=a.size(),nc=h.size();std::vector<int>rows,cols;
        for(const auto&cell:cells){int v=cell.front();
            (v<nr?rows:cols).push_back(v<nr?v:v-nr);}
        std::string s;s.reserve(3+nr+nc+2*nr*nc);
        s.push_back(char(P));s.push_back(char(nr));s.push_back(char(nc));
        for(int u:rows)s.push_back(char(a[u]));for(int v:cols)s.push_back(char(h[v]));
        for(int u:rows)for(int v:cols)s.push_back(char(m[u*nc+v]));
        for(int u:rows)for(int v:cols)s.push_back(char(all[u*nc+v]));
        return s;
    }
    static std::string canonical_search(
            const std::vector<int>&a,const std::vector<int>&h,
            const std::vector<int>&m,const std::vector<int>&all,
            std::vector<std::vector<int>>cells,int P) {
        refine(a,h,m,all,cells);int chosen=-1;
        for(int i=0;i<int(cells.size());++i)if(cells[i].size()>1 &&
            (chosen<0||cells[i].size()<cells[chosen].size()))chosen=i;
        if(chosen<0)return discrete_code(a,h,m,all,cells,P);
        // Exact individualization can be factorial on stars.  If refinement
        // does not make the state discrete, retain the original ordered key.
        // This sacrifices only cross-tree memo hits, never correctness.
        std::string s;s.reserve(4+a.size()+h.size()+2*m.size());
        s.push_back('O');s.push_back(char(P));
        s.push_back(char(a.size()));s.push_back(char(h.size()));
        for(int z:a)s.push_back(char(z));for(int z:h)s.push_back(char(z));
        for(int z:m)s.push_back(char(z));for(int z:all)s.push_back(char(z));
        return s;
    }
    static std::string key(const std::vector<int>&a,const std::vector<int>&h,
                           const std::vector<int>&m,
                           const std::vector<int>&all,int P) {
        std::map<std::pair<int,int>,std::vector<int>>groups;
        int rows=a.size();
        for(int u=0;u<rows;++u)groups[{0,a[u]}].push_back(u);
        for(int v=0;v<int(h.size());++v)groups[{1,h[v]}].push_back(rows+v);
        std::vector<std::vector<int>>cells;
        for(auto&[label,vertices]:groups)cells.push_back(std::move(vertices));
        return canonical_search(a,h,m,all,std::move(cells),P);
    }
    ...
    static bool sealed_is_forest(const std::vector<int>&m,
        ...
        auto root=[&](int x){int y=x;while(parent[y]!=y)y=parent[y];
            while(parent[x]!=x){int z=parent[x];parent[x]=y;x=z;}return y;};
        for(int u=0;u<rows;++u)for(int v=0;v<cols;++v) {
            int raw=all[u*cols+v],left=m[u*cols+v];
            if(raw>2 || left>raw || ((raw-left)&1))return false;
            if(raw==left)continue;
            int x=root(u),y=root(rows+v); if(x==y)return false;
    ...
    i128 rec(const std::vector<int>&a,const std::vector<int>&h,
             const std::vector<int>&m,const std::vector<int>&all,int rem) {
        ...
        // In a leading connected quotient, the P covariance edges form a
        // tree on P+1 vertices.  Consequently every final row/column cell
        // contains exactly two raw W edges (the members of one Wick pair).
        // Quotienting only coarsens cells, so >2 remaining raw edges in one
        // current cell is impossible.  Exactly two are forced to pair with
        // each other: partnering either with an outside edge would put at
        // least three raw edges in its final cell.
        int forced=-1;
        if(!sealed_is_forest(m,all,a.size(),h.size()))return 0;
        for(int q=0;q<int(m.size());++q) {
            if(all[q]>2)return 0;
            if(m[q]==2)forced=q;
        }

        // A row class of odd total a-degree must still be merged.  If it is
        // no longer incident to an unpaired W, that is impossible and its
        // centered Gaussian moment will vanish at every descendant.
        for(int u=0;u<int(a.size());++u)if(a[u]&1) {
            bool active=false;
            for(int v=0;v<int(h.size());++v)active|=m[u*h.size()+v]!=0;
            if(!active)return 0;
        }

        std::string k=key(a,h,m,all,P); auto found=memo->find(k);
        if(found!=memo->end())return found->second;
        int rows=a.size(),cols=h.size(),q0=-1;
        if(forced>=0)q0=forced;
        else {
            for(int q=0;q<int(m.size());++q)if(m[q]){q0=q;break;}
        }
        int u0=q0/cols,v0=q0%cols; auto first=m; --first[q0]; i128 ans=0;
        int partner_begin=forced>=0?q0:0;
        int partner_end=forced>=0?q0+1:int(first.size());
        for(int q1=partner_begin;q1<partner_end;++q1)if(first[q1]) {
            int mult=first[q1],u1=q1/cols,v1=q1%cols;
            auto aa=a,hh=h,mm=first,total=all; --mm[q1];
            merge_rows(u0,u1,aa,mm,cols);
            {auto dummy=a;merge_rows(u0,u1,dummy,total,cols);}
            merge_cols(v0,v1,hh,mm,aa.size(),cols);
            {auto dummy=h;merge_cols(v0,v1,dummy,total,aa.size(),cols);}
            ans += i128(mult)*rec(aa,hh,mm,total,rem-2);
        }
        memo->emplace(std::move(k),ans); return ans;
    }
    i128 evaluate(const Component&c,std::unordered_map<std::string,i128>*shared=nullptr) {
        ...
        return rec(a,h,m,m,c.edges.size());
    }
};
...
int main(int argc, char **argv) {
    ...
    int thread_count=1;
#endif
    std::vector<std::unordered_map<std::string,i128>> shared_memos(thread_count);
    ...
    for (int idx=0; idx<int(unique_components.size()); ++idx) {
        ...
        component_values[idx]=c.edges.size() >= 0
            ? TypePairingCounter().evaluate(c,&shared_memos[tid])
