...
struct TypePairingCounter {
    ...
    static std::string key(const std::vector<int>&a,const std::vector<int>&h,
        ...
        std::string s; s.reserve(2+a.size()+h.size()+2*m.size());
    ...
    i128 rec(const std::vector<int>&a,const std::vector<int>&h,
        ...
        std::string k=key(a,h,m,all); auto found=memo.find(k);
        if(found!=memo.end())return found->second;
        int rows=a.size(),cols=h.size(),q0=-1;

        auto occupancy_valid=[&](int q,int p) {
            if(!m[q]||!m[p]||(q==p&&m[q]<2))return false;
            int u0=q/cols,v0=q%cols,u1=p/cols,v1=p%cols;
            int newV=V-(u0!=u1)-(v0!=v1);
            if(newV<target||newV-2*(pairs-1)>target)return false;
            int nr=rows-(u0!=u1),nc=cols-(v0!=v1);
            int counts[625]{};
            int du=std::max(u0,u1),ku=std::min(u0,u1);
            int dv=std::max(v0,v1),kv=std::min(v0,v1);
            for(int u=0;u<rows;++u)for(int v=0;v<cols;++v)if(all[u*cols+v]) {
                int uu=u,vv=v;
                if(u0!=u1){if(uu==du)uu=ku;else if(uu>du)--uu;}
                if(v0!=v1){if(vv==dv)vv=kv;else if(vv>dv)--vv;}
                if((counts[uu*nc+vv]+=all[u*cols+v])>2)return false;
            }
            return true;
        };
        if(forced>=0)q0=forced;
            ...
            int best=1000000;
            for(int q=0;q<int(m.size());++q)if(m[q]) {
                int choices=0;
                for(int p=0;p<int(m.size());++p)
                    if(occupancy_valid(q,p))choices+=m[p]-(p==q);
                if(choices<best){best=choices;q0=q;}
            }
            if(best==0){memo.emplace(std::move(k),0);return 0;}
        } else {
        ...
        for(int q1=partner_begin;q1<partner_end;++q1)if(first[q1]) {
            if(!occupancy_valid(q0,q1))continue;
