...
This note records the current claim level after the exact order-eleven peeling
calculation, the finite-width experiments, and the positive-operator audit.  It
supersedes the opening status paragraph of `REPORT.md`.
...
`exact_peeling/SECTOR_ENGINE.md`.
...
`exact_peeling/D13_ATTEMPT.md` records the checkpoint hash, strict order-nine
...
`exact_peeling/MATROID_WICK_AUDIT.md`.
...
are in `exact_peeling/TWO_HIT_CHARGING_AUDIT.md`.
