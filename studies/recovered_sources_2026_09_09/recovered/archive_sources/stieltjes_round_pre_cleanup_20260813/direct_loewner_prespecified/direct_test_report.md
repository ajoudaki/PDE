...
```text
python simulate_loewner.py --output run_20260812
```
...
The failed command log is `run_20260812/run.log`. The later command

```text
python diagnose_blowup.py --output failure_diagnostic_20260812
```
...
`failure_diagnostic_20260812/failure_summary.json`.
...
- `run_20260812/run.log`: aborted primary command log.
- `diagnose_blowup.py`: post-failure escape diagnostic.
- `failure_diagnostic_20260812/trajectory_escape.csv`: raw per-trajectory
  initial values, crossing times, survival flags, and surviving `K` values.
- `failure_diagnostic_20260812/escape_*.npz`: exact raw arrays.
- `failure_diagnostic_20260812/failure_summary.json`: summaries and control
  matrices.
- `failure_diagnostic_20260812/manifest.json`: byte sizes and SHA-256 hashes.
