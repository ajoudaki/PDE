# A class of nonlinear activations with global limits for three separated inputs
...
Fix a function \(\psi\in C^2(\mathbb R)\) that is nonconstant and satisfies
\[
 \|\psi\|_\infty\le1,\qquad \|\psi'\|_\infty\le1,
 \qquad \|\psi''\|_\infty\le1.                              \tag{M.1a}
\]
No parity, sign, monotonicity, analyticity, or limits at infinity are
assumed of \(\psi\). Any nonconstant bounded \(C^2\) function with
bounded first and second derivatives can be put in this form by
dividing it by \(\max\{1,\|\psi\|_\infty,\|\psi'\|_\infty,
\|\psi''\|_\infty\}\); the corresponding scalar factor is absorbed
in the perturbation coefficient.

We will choose two constants \(a=a_{\delta,\psi}\ge2\) and
\(e_{\delta,\psi}>0\).
For any fixed \(e\in(0,e_{\delta,\psi}]\), use the same activation
\[
                   \phi(z)=a(1+z)+e\psi(z)                 \tag{M.2}
\]
...
and on \(\psi\), but not on the particular admissible data.
...
field is continuously differentiable and locally Lipschitz. Its energy identity gives
...
everywhere-defined continuously differentiable maps in (M.4)--(M.7).
...
**Theorem M.1.** For each \(0<\delta\le3/2\) and each nonconstant
\(\psi\) satisfying (M.1a), there are finite constants
\(a_{\delta,\psi}\ge2\) and \(e_{\delta,\psi}>0\) depending only on
\(\delta,\psi\), such that, for every fixed \(e\in(0,e_{\delta,\psi}]\),
...
3. There is a positive constant \(\eta_*=\eta_{\delta,\psi}\), specified
   below and uniform over admissible data and training time, such that
   every sample and hidden layer obeys
   \[
...
Here is an explicit selection rule. Choose an integer \(r_\psi\ge1\)
for which
\[
 J_\psi=\inf_{\alpha,\beta\in\mathbb R}
       \int_{-r_\psi}^{r_\psi}[\psi(x)-\alpha-\beta x]^2\,dx>0,
 \qquad
 c_\psi=\frac{e^{-r_\psi^2/2}}{\sqrt{2\pi}}J_\psi.           \tag{M.20a}
\]
Part G proves that such an integer exists. These constants use only
one-dimensional integrals of the selected function, not a trained
trajectory or a data-dependent minimum. Set
\[
 \begin{split}
 \tau_\psi&=\min\{1/2,\sqrt{c_\psi/5}/6\},\qquad
 \lambda=\delta^2/256,\\
 a=a_{\delta,\psi}
   &=\max\left\{2,\frac{2000}{\sqrt\lambda},
             \left(\frac{10^{12}}{\lambda^2\tau_\psi}\right)^{1/3}
                                                       \right\},\\
 \eta_*&=\frac{c_\psi}{5a^2},\qquad t_*=\frac{\tau_\psi}{a},
 \qquad S=\frac{12}{\lambda a^6},\\
 e_{\delta,\psi}
   &=\tfrac12\min\{1,e_*(a,12,S),(10^{10}a)^{-1}\}.
 \end{split}                                                \tag{M.21}
\]
We write \(e_*(a,B,S)=\epsilon_*(a,B,S)\), where (R.90) defines the
latter by a finite explicit chain of estimates. Every number in
(M.21) is finite and strictly positive. A different valid interval
in (M.20a) merely supplies a different sufficient selection rule.

The wider class does not in general have a positive regression
margin uniform over all Gaussian variances. Accordingly the
coefficient \(\eta_{\delta,\psi}\) in (M.19) is allowed to depend on
\(\delta,\psi\). Part A gives infinite-dimensional subclasses with
one common recipe for activation parameters and a positive margin
independent of \(\delta\), as well as explicit examples.

In particular the theorem applies to the requested input class
\(-1+\delta<\Gamma_{ij}<1-\delta\) for \(i\ne j\) and
\(0<\delta<1\). The proof establishes the stronger one-sided
separation (M.1), including singular input Gram matrices.
...
\[
\phi(z)=a(1+z)+e\psi(z),\qquad a\ge1,\quad 0\le e\le1.
\tag{F.1}
\]
...
For the gain network let \(g(z)=\psi'(z)\), and choose fixed smooth clips with
\[
...
\[
 \phi_\epsilon(z)=a(1+z)+\epsilon\psi(z),\quad g(z)=\psi'(z),
...
\[
 ...
 \tag{R.4}
\]
Here \(|g|\le1\), \(|g'|\le1\), and \(|\psi|\le1\le2\).
All the following inequalities use these absolute bounds; \(g\) and
\(g'\) may have either sign. The response formulas keep their actual
signed expectations. At fixed cap these coordinate instructions are continuously differentiable and globally Lipschitz: the two partial derivatives of \(D\) are bounded by \(2\epsilon R\) and \(a+\epsilon\).
...
\[
 D_R(z,q)=aq+e\psi'(z)\tau_R(q)                         \tag{G.9}
\]
...
\(0<\lambda\le9/1024\), these constants obey
\[
 D_S<\min\{1/2,\lambda/(3\cdot10^7)\},\quad C_S<1,\quad
                  6\cdot10^6C_S^2<\lambda/4.               \tag{G.18}
\]
The cubic lower bound on \(a\) in (M.21), together with
\(t_*=\tau_\psi/a\), gives
\[
...
\[
 \phi(z)=a(1+z)+e\psi(z),\qquad a\ge1,\quad 0<e\le1,
 \qquad d_\phi=\phi'.
\]
...
write
\[
 D_R(z,q)=aq+e\psi'(z)\tau_R(q),\qquad
...
\[
 e[\psi'(z)-\psi'(\bar z)]\tau_R(\bar q)
 +e\psi'(z)[\tau_{R'}(\bar q)-\tau_R(\bar q)].
\]
The derivative of \(z\mapsto\psi'(z)\) has absolute value at most one; \(|\tau_R|\le2R\); and the two clips agree when \(|\bar q|\le R\), while both have absolute value at most \(|\bar q|\). This proves (V.10).
...
\(\phi(z)=a(1+z)+e\psi(z)\), and put \(d_i^\ell=\phi'(Z_i^\ell)\).
The activation parameters in this part satisfy \(a\ge2\) and \(0<e\le1\), with the additional quantitative upper bound specified in Section N.5. Define
\[
...
\[
 ...
 \tag{N.10}
\]
The initial Gaussian projection and augmented-Gram inequality (G.1), (G.4) imply \(Q_1\succeq (a^2/4)(\Gamma+\mathbf1\mathbf1^T)\succ0\), and \(Q_2\succeq (a^2/4)Q_1\succ0\). The initialized \(Z^3\) is therefore a nondegenerate centered three-dimensional Gaussian, with covariance \(Q_2\), and has a strictly positive density on \(\mathbb R^3\).
...
\[
 ...
 \tag{N.11}
\]
Indeed the identity first holds almost surely, and full support and continuity extend it to every point. The first factor has no open zero set: its derivative in coordinate \(i\) is \(p_i\phi'(z_i)\), which is never zero because \(p_i\ne0\) and \(\phi'\ge a/2\). Its nonzero set is therefore dense. The second factor vanishes on that dense set and hence everywhere. Differentiate it in coordinate \(i\) to obtain \(v_i\phi''(z_i)=0\) for every \(z_i\). Since \(\phi''=e\psi''\) is not identically zero when \(e>0\), every \(v_i=0\). Indeed \(\psi''\equiv0\) would make \(\psi\) affine, and boundedness would then make it constant, contrary to (M.1a). Thus
\[
                                  S_3\succ0.
 \tag{N.12}
\]
...
\[
 ...
 \ge (a^2/4)\lambda_{\min}(S_3)\|v\|_2^2.
 \tag{N.15}
\]
...
\[
 ...
 \succeq (a^2/4)\lambda_{\min}(S_2)I_3.
 \tag{N.16}
\]
...
\[
 d\|V^1\|_2^2
 \ge\frac{(a^2/4)\lambda_{\min}(S_2)}d\sum_i p_i^2\|x_i\|_{\mathbb R^d}^2
 =(a^2/4)\lambda_{\min}(S_2)\sum_i p_i^2>0.
 \tag{N.17}
\]
...
\[
 ...
 \ge (a^2/4)\lambda_{\min}(S_2)\sum_i\Gamma_{ji}^2p_i^2
 \ge (a^2/4)\lambda_{\min}(S_2)p_j^2>0.
 \tag{N.19}
\]
The last step uses \(\Gamma_{jj}=1\). Since \(d_j^1\ge a/2\), the corresponding feature direction \(d_j^1U_j^1\) is nonzero as well. This covers singular \(\Gamma\) without its inverse and without requiring nonzero affine bottom motion.
...
The following estimates use \(e\le1\), \(\|A\|,\|B\|\le10\), \(\|Z_j^1\|_2=1\), \(\|\psi\|_\infty\le1<2\), and \(\sum_i|p_i|=1\). All bounds are per sample; the bound on \(h^3\) also bounds \(H\).
\[
\begin{array}{c|c|c}
...
\[
 ...
 \tag{N.53}
\]
For the first scalar inequality, \(18\sqrt5<45<50\); for the second, \(18\sqrt{7/3}<36<50\). The triangle inequality therefore proves \(U_j^2\ne0,U_j^3\ne0\) for every sample at the chosen positive nonlinearity. Their feature directions are nonzero as well because \(\phi'\ge a/2\). The activation rule (M.21) is smaller than the cutoff (N.52). Together with Section N.2 this proves nonzero preactivation and feature directions in every layer and sample.
...
This gives infinite-dimensional neighborhoods of
