...
number of factors `H`, (C4.E3) gives `M_j=2^r 4^s`. On the real domain all
...
gives the stated rational envelopes. These finite-sum inequalities can be checked by integer arithmetic.
...
Apply (C4.E8) with `(b,c)=(1,1)` for `H`, `(1,2)` for `D`, and `(2,5)`
for `E`. The exact constants needed in (C4.E6) are consequently
...
in the original derivation. (C4.E9) certifies the full response means; it
...
The retained source is [certificate_kernel.cpp](../code/tools/two_layer_risk/certificate_kernel.cpp).
...
The rational calculation in [angle_error_bound.py](../code/tools/two_layer_risk/angle_error_bound.py) gives a sharper bound;
...
The identity `e_x h_x=-phi''(Z_x)/2` improves the last term of (C4.A6).
...
to the absolute dyadic grid and then multiplied by large powers. This order and the trigonometric-width gate keep the intervals
...
contracts are retained in the run. The supplied-state kernel checks independently test this implementation;
