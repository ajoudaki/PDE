...
KERNEL = SOURCE.with_name('certificate_kernel.cpp')
...
def grid(matrix, target):
    """Dyadic grid satisfying (C4.E1)--(C4.E5) and (C4.D2)--(C4.D3)."""
...
def _validate_request(output, target):
    """Validate the public boundary without creating output or starting work."""
    if sys.flags.optimize:
        raise RuntimeError('Certificate checks require Python without -O or -OO')
    if type(target) is not int:
        raise TypeError('target must be a Python integer (26 or 30)')
    if target not in (26, 30):
        raise ValueError('target must be 26 or 30')
    if sys.platform != 'linux':
        raise RuntimeError('The certificate execution contract requires Linux')
    if sys.float_info.radix != 2 or sys.float_info.mant_dig != 53:
        raise RuntimeError('Python binary64 floating-point arithmetic is required')
    raw = os.fspath(output)
    if not isinstance(raw, str):
        raise TypeError('output must be a string or string-valued path-like object')
    if not raw or '\x00' in raw:
        raise ValueError('output must be a nonempty path without NUL characters')
    proposed = Path(raw).expanduser()
    if proposed.is_symlink():
        raise FileExistsError('output must be fresh and cannot be a symbolic link')
    out = proposed.resolve()
    if out.exists():
        raise FileExistsError(f'output must be fresh: {out}')
    return out


def certify(output, target=26):
    """Return a fresh exact coefficient enclosure and retain its audit files.

    output must name a nonexistent directory; target is the integer 26 or 30.
    Execution requires Linux, NumPy, g++ with the documented arithmetic contract,
    and non-optimized Python. The fixed model and all numerical inputs are private.
    A fresh worker isolates thread settings and CPU accounting from the caller.
    Its progress is printed; the returned JSON-compatible dict is independently
    mutable. See README.md for mathematical scope, errors and resource limits.
    """
    out = _validate_request(output, target)
    env = os.environ.copy()
    for name in ('OPENBLAS_NUM_THREADS', 'OMP_NUM_THREADS', 'MKL_NUM_THREADS',
                 'NUMEXPR_NUM_THREADS'):
        env[name] = '1'
    command = [sys.executable, '-B', str(SOURCE), '--output', str(out),
               '--target', str(target), '--_worker']
    subprocess.run(command, env=env, check=True)
    return json.loads((out/'result.json').read_text())


def run(args):
    """Private worker; all public computation is launched through certify."""
    out = _validate_request(args.output, args.target)
    sources={SOURCE.name:sha(SOURCE), KERNEL.name:sha(KERNEL)}
    out.mkdir(parents=True,exist_ok=False)
    compiler=['g++','-O3','-std=c++17','-fno-fast-math','-ffp-contract=off',
              str(KERNEL),'-o',str(out/'certificate_kernel')]
    ...
    try:
        meta['environment']['compiler']=subprocess.check_output(['g++','--version'],text=True).splitlines()[0]
...
if __name__=='__main__':
    ...
    if args._worker:
        ...
        certify(args.output,args.target)
