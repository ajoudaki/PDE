...
The [maintained theorem library](../../docs/README.md) now provides separate,
self-contained chapters. This report remains the broader research-state ledger;
the [promotion record](../repository_refactor_2026_09_09/PROMOTION_LEDGER.md)
distinguishes included proofs, editorial backlog and unresolved dependencies.

9 September 2026 access-recovery edition: source reconciliation complete; three fresh isolated final document-only reviews CLEAN. Source-proof and reading-scope limitations remain explicit. [Exact review and version record](REVIEW_STATUS.md).
...
The six source reports provide proof skeletons, precise assumptions, original line links and hashes: [MFP calculus](source_audits/MFP_CALCULUS.md), [Stieltjes/quadratic](source_audits/STIELTJES_QUADRATIC.md), [linear/representation](source_audits/LINEAR_AND_REPRESENTATION.md), [older nonlinear/ResNet/RMS](source_audits/OLD_NONLINEAR_RESNET_RMS.md), [primary literature](source_audits/PRIMARY_LITERATURE.md), and [recent global limits](source_audits/GLOBAL_NONLINEAR_FRONTIER.md). The coordinator's [continuation audit](source_audits/ROOT_CONTINUATION_AND_OPERATOR.md) and [elementary logical bridges](source_audits/LOGICAL_BRIDGES_AND_SCIENTIFIC_SCOPE.md) are additional supporting records.

The earlier [four-task master](../four_thread_consolidation/CONSOLIDATED_RESULTS.md) was read completely and is preserved unchanged as a historical consolidation. Its recovered source archive is valuable, but neither the archive nor its review counts prove chronological completeness. A further 127 historical Markdown files from the destination arctangent continuation have been preserved unchanged under [sources/arctan_destination](sources/README.md), with a [hash manifest](sources/ARCTAN_DESTINATION_SHA256.txt). Invalid drafts are preserved for provenance, not promoted.

**History-access recovery completed.** Following the user's permission change, all 43 located rollout files belonging to the 19 requested PDE-2 task identities were opened and fully parsed: 620,486,061 bytes, with no JSON parse errors. Six fresh isolated source auditors and the coordinator read the full deduplicated mathematical dialogue of those identities, including all located alternate branches and seven referenced mathematical attachments, and inspected newly operative proofs and corrections. The coordinator read all eight recovery reports completely and independently checked selected decisive sources. Exact full-read versus selected-read coverage is recorded in the [recovery directory](access_recovery_2026_09_09/README.md). This is not a human read of encrypted/admin records or every raw tool output, nor a fresh recursive reaudit of every unchanged ancestor manuscript. A second permission update also opened all four specifically reported denied temporary files: the three L2/L3 originals exactly match their already fully read archives, and the original TP-III TeX proof source is now readable. Its dedicated source check is separate from byte verification. [Temporary recovery record](access_recovery_2026_09_09/TMP_ACCESS_RECOVERY.md). Equal paths across hosts still do not establish byte identity. Access and mathematical proof gaps remain distinct.
...
The block-metric proof is particularly informative: it first computes a frozen-bottom two-variable polynomial recursion, then derives the positive-$\alpha$ chronological Gaussian recurrence and an exact determinant polynomial whose sign persists on a nonzero interval. Thus the negative is not merely an artifact of freezing a layer. It disproves an activation/metric-universal Stieltjes law, not operator-valued population evolution. [Full metric proof](../stieltjes_resolution/BLOCK_METRIC_RESOLUTION.md), [all-block extension](../stieltjes_resolution/POSITIVE_ALPHA_JET_DERIVATION.md).
...
is a smooth nonsingular autonomous two-dimensional ODE with unique solution $(t,g(t))$. This is not a proposed neural closure; it disproves the logical implication “zero radius means no smooth ODE.” Analytic vector fields with analytic readouts obey a different local theorem. Formal jets also need a trajectory-identification bridge before they establish actual trajectory regularity. [Formal no-go proof](../quadratic_nonclosure/approximate_single_source_conjecture_resolution.md), [positive-compiler boundaries](../quadratic_nonclosure/adversarial_audit_report.md).
...
Repeated derivatives generate connected noncommutative contraction paths of unbounded degree. Products of a fixed alphabet produce disconnected unions, not all new connected types; spatial differentiation changes coefficient functions, not that alphabet. The analytic version requires a full neighborhood of the zero-state jets. The negatives do not exclude infinite-dimensional current operators, arbitrary encodings, approximations, or a curve fitted to one orbit. A translation PDE initialized with the entire future output profile is formally possible but preloads the answer. [PDE boundary proof](../deep_linear_closure_boundary/PDE_CLOSURE_BOUNDARY.md).
...
Principal sources: [L2 baseline](../four_thread_consolidation/inherited_baselines/L2_ONE_SAMPLE_ARCTAN_GLOBAL_PROOF.md), [broad L2 and affine exception](../four_thread_consolidation/expanded_sources/arctan_two_primary/GLOBAL_ACTIVATION_TRANSFORM_EXTENSION.md), [all-depth one-sample proof](../four_thread_consolidation/expanded_sources/SHIFTED_ATAN_ALL_DEPTHS/GENERAL_DEPTH_SELF_CONTAINED_PROOF.md), [two-sample offset](../two_sample_separated_angle_theorem/PROOF.md), [exponent-two refinement](../odd_activation_lower_powers_three_inputs/TWO_INPUT_PROOF.md), [two-sample all-depth proof](../two_sample_odd_activation_depth56/PROOF.md), [shape extension](../two_sample_activation_design/PROOF.md), [three-sample all-depth shape proof](../activation_class_all_depths/MANUSCRIPT.md), [odd-gain proof](../three_sample_odd_gain_all_depths/PROOF.md).
...
**Important shared audit dependency.** TP III E.15 / TP IV G.4 state rank-free pseudo-Lipschitz fixed-program convergence, including transposes and scalar feedback. The literature auditor read TP III Appendices K–N completely and checked corrections to conditioning, normalization and moment exponents. Appendix N's stronger conditional-coefficient/all-moment argument for vanishing directions with changing Gram rank remains unclosed in this audit. A bounded matrix operator norm only proves the corresponding $L^2$ smallness, not all higher moments. This is not a counterexample to the master theorem. It means that a project proof importing that full extension is **dependency-qualified under the user's unusually strong proof-audit requirement** until the needed specialization is repaired or proved directly. The source-specific disposition is recorded rather than silently treating all published imports as flawless. [Exact proof locations and qualifications](source_audits/PRIMARY_LITERATURE.md:147).

**The recovered original TeX does not remove this qualification.** A fresh independent source auditor read all 3,970 logical lines of the proof file, including comments and EOF, together with the required local statements and proof dependencies. It matches the relevant v3 published text, not a corrected hidden version. The load-bearing issue is Lemma **N.2**: the asserted all-moment negligibility of $A\Delta h$ still needs control of the conditional mean after disappearing query directions and scalar dependence. Core rank stability does not automatically control the enlarged history; coefficient convergence is asserted rather than proved there. The separate Lemma N.3 is the elementary pseudo-Lipschitz perturbation step before matrix multiplication, not the missing estimate. Local algebraic corrections and Gaussian-image high-moment bounds survive but do not supply that conditional-mean bound. This is an available-proof audit gap, not a missing file, a hidden rank hypothesis in E.15, or an actual counterexample. The parameterless theorem has its own rank/zero-stability induction and remains separately scoped. [Full TeX-level audit, dependencies and source hashes](access_recovery_2026_09_09/TMP_TP3_AUDIT.md).
...
Numbers match the user's list and [exact identity index](TASK_INDEX.md). A task can contribute a proof, a correction, a method, or recovery work; its title does not determine theorem scope. “Recovered” below refers to the identified local/exported bytes, not a claim of current remote filesystem identity.
...
| 10. Test nonlinear mean-field scaling | Full recovered dialogue corrects the old attribution: theoretical scale balance, a `2+sin` candidate, initial-activity identities and failure of ordinary neuron/one-edge closure. The final compact-time operator-limit problem remains open. This was not the quadratic/ReLU mesh experiment. [Direct recovery audit](access_recovery_2026_09_09/ROOT_RECOVERY.md) |
...
The focused covariant-Schur/import follow-ups and the special-variant/original-L3-local source audits are complete at their recorded scopes. The permission-recovery pass closes the located PDE-2 mathematical-dialogue gaps and adds the operative source checks recorded in its eight reports and temporary-source follow-up. Independent document-only review and correction are tracked separately for each edition. Selected ancillary/dependency manuscripts not recursively reaudited, unresolved temporary lookup targets, unrederived large coefficient tables and unclosed imported proof chains remain explicit. All specifically reported temporary permission barriers are now removed; this does not recover vanished drafts, authenticate arbitrary same-path remote copies or prove a missing lemma. A clean review of an accurately qualified consolidation does not make those mathematical premises proved or turn the main global/generalization objective into a completed result.
