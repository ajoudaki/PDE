...
> **Compact-edition note.** This is the scientific report from the original
> 689 MB evidence release. The present source-first bundle includes the
> complete regeneration code, compact CSV/JSON summaries, and central
> figures, but intentionally omits raw trajectory and bootstrap-distribution
> `.npz` files. Statements below that describe those raw files as archived or
...
`audits/final_adversarial_pde_audit.md`. The earlier
`audits/adversarial_audit.md` is preserved as the historical audit
that rejected the old finite-matrix surrogate and specified the acceptance
gates.

The independent statistical report, code, compact bootstrap tables, and
compact discrepancy summaries are under `audits/statistical_audit/`.
