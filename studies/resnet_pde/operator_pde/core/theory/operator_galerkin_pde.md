...
\(D_r^\ell=\operatorname{diag}\sech^2(z_r^\ell)\), and
...
\[
\beta_r(s,\theta,w,t)
:=\sech^2(z_r(s,\theta,w,t))p_r(s,\theta,t).
\]
...
For \(d=m=3\), \(P=5\) contains the constant, three input-map coordinates,
and the readout coordinate. The augmented characteristic representation
...
The bundle supplies direct numerical evidence for all but the theorem:
paired dense runs show both forward and adjoint conditional variances
decaying as \(1/L\), including after training, while the PDE curves converge
in \(N,M,R\) and agree with increasingly wide dense ensembles.
