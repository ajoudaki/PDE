...
## Further correlated-data nonlinear families

Fragments D–J below use the complete common setup and A.1–A.4 probability,
value, norm and scalar-differentiation lemmas in [the nonlinear chapter](global_nonlinear.md),
under “Nonlinear activation, separation, and continuation additions”.
Their first-storage conversion and half-sum loss are stated there and
again in the individual models below. III.F and III.V refer to the complete
Gaussian and observation proofs earlier in this chapter. Lettered equation
numbers are local to these fragments; they do not rename Parts I–V.
...
The complete statements and proofs are in [finite controls](finite_optimization_and_controls.md),
“E. Moderate-sine physical controls”, equations (E.1)–(E.15). That section
uses exactly this calibrated coefficient 2/5, three hidden layers, two
binary-labelled RMS-unit samples, half-sum loss, canonical tiny finite
Gaussian readout and the raw GD step n^-2. It proves global finite GF,
compact-horizon raw-GD energy bounds, weak same-layer path-law tightness,
and strong raw-state, field and velocity endpoints for an already existing
strong population path on its specified action spaces. It also proves the
ambient gate-multiplier obstruction. These results require no global sine
population theorem. The correlated-control bracket is proved in J.1 below.
