...
\(n\times n\). In this section set
\(\langle v,w\rangle_n=v^Tw/n\) and \(\|v\|_n=\|v\|_2/\sqrt n\).
Matrix norms are ordinary operator norms. Define
\[
 h^\ell=\phi(z^\ell),\quad z^2=W^{(2)}h^1,\quad
 z^3=W^{(3)}h^2,\quad f=\langle a,h^3\rangle_n,
\]
\[
...
\[
 ...
 R_1(s)&=\int_0^s h^1(u)\langle b'(u),b(s)-b(u)\rangle_n\,du,
 \qquad z^1=F^{-1}(X^1).
 \end{split} \tag{13.4}
\]
...
\[
 ...
 z^2(s)&=W^{(2)}_0h^1(s)+\int_0^s b'(u)
                         \langle h^1(u),h^1(s)\rangle_n\,du,\\
 z^3(s)&=W^{(3)}_0h^2(s)+\int_0^s\delta^3(u)
                         \langle h^2(u),h^2(s)\rangle_n\,du,\\
 a(s)&=a_0+\int_0^s h^3(u)\,du,\\
 q^2(s)&=(W^{(3)}_0)^T\delta^3(s)+\int_0^s h^2(u)
                         \langle\delta^3(u),\delta^3(s)\rangle_n\,du,\\
...
and \(|\phi''|\le2\). Thus
\(\|\delta^3\|_n\le B\), \(\|q^2\|_n\le B^2\), and
\(\|b'\|_n=\|\delta^2\|_n\le B^2\). Direct differentiation gives
\[
 \begin{aligned}
 (h^1)'&=\phi'(z^1)^2\odot(W^{(2)})^T\delta^2,\\
 (z^2)'&=\|h^1\|_n^2\delta^2+W^{(2)}(h^1)',\\
 (h^2)'&=\phi'(z^2)\odot(z^2)',\\
 (z^3)'&=\|h^2\|_n^2\delta^3+W^{(3)}(h^2)',\\
 (\delta^3)'&=h^3\odot\phi'(z^3)
                  +a\odot\phi''(z^3)\odot(z^3)'.
 \end{aligned} \tag{13.7}
\]
Consequently their normalized bounds, in the same order, are
\[
 B^3,\quad Z_2=(c^2+B^2)B^2,\quad Z_2,\quad
 Z_3=c^2B+BZ_2,\quad c+2BZ_3.
 \tag{13.8}
\]
In particular every argument in the table has \(\|v'\|_n\le C\),
where \(C=\max\{1,B^2,B^3,Z_2,c+2BZ_3\}\) depends only on \(B\).
Integration proves \(\|v(s)-v(t)\|_n\le C|s-t|\).
...
\(\|h\|_n,\|\widetilde h\|_n\le B_h\),
\(\|h'\|_n,\|\widetilde h'\|_n\le L_h\), and
\(\|b'\|_n,\|\widetilde b'\|_n\le L_b\).
Let \(e_b=\sup_s\|b-\widetilde b\|_n\) and
\(e_h=\sup_s\|h-\widetilde h\|_n\).
...
\]
The rank-one identity \(\|vw^T/n\|_{\rm op}=\|v\|_n\|w\|_n\)
...
\[
 \sup_s\|R-\widetilde R\|_n
 \le SL_b e_M+2SL_bB_h e_b. \tag{13.11}
\]
These are the lower-memory bounds for \(h=h^1\). For top memory,
if both \(h^2\) paths have norm at most \(B_2\) and both
\(\delta^3\) paths have norm at most \(D_3\), the direct difference
of its two rank-one integrands gives
\[
 \sup_s\|M_3-\widetilde M_3\|_{\rm op}
 \le SB_2\sup_s\|\delta^3-\widetilde\delta^3\|_n
       +SD_3\sup_s\|h^2-\widetilde h^2\|_n. \tag{13.12}
\]
...
\[
 ...
 \tag{13.13}
\]
The first summand has norm at most \(\|q^2-\widetilde q^2\|_n\).
Splitting the second at any \(Q>0\) gives
\[
 \|[\phi'(z^2)-\phi'(\widetilde z^2)]\odot\widetilde q^2\|_n
 \le 2Q\|z^2-\widetilde z^2\|_n
    +2\|\widetilde q^2\mathbf1_{|\widetilde q^2|>Q}\|_n.
 \tag{13.14}
\]
...
\(\widetilde q=\sqrt n e_1\) also makes the product on the left have
norm \(1/2\), although \(\|z-\widetilde z\|_n=1/\sqrt n\).
