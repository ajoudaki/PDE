# Standalone caps/pullback validation

Captured deterministic `make check` output from the 25-file scientific copy.

```text
python -B code/tools/check_library.py
Library boundary and local links checked: 23 files.
make test
make[1]: Entering directory '/tmp/pde_caps_standalone'
...
OK
make[1]: Leaving directory '/tmp/pde_caps_standalone'
```
