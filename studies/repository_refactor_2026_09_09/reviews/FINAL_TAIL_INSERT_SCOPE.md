...
## Initialized, conditional and local dynamics

The letters A–E and their equation numbers are local to this section.
...
mobilities specialized to \(\kappa_1=\kappa_2=\kappa_3=\kappa_4=1\).
Initialize first weights with variance one, hidden matrices
...
\]
The complete block-row norms of \(\mathcal R\) and \(\mathcal T\) are at most
\(E\); consequently \(\|U\|_{\rm row}\le\alpha SE\). Here is a check that
...
with unit initial feature variance at layer zero, zero population
readout, and unit mobility multipliers in the product raw metric.
At an initial Gaussian layer whose common preactivation variance
is \(q_{\ell-1}>0\), put
\[
