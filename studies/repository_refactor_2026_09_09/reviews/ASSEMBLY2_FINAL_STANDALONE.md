# Standalone scientific validation

The 27-file candidate edition was assembled using only the scientific allowlist, without studies, data, archived arrays, exporter files or exporter tests. It passed the dependency/local-link checker and all 90 deterministic tests under Python 3.10.12 and NumPy 1.26.4. The frozen-bottom guide example passed. All input hashes remained unchanged. No training experiment or historical campaign was run.

```text
python -B code/tools/check_library.py
Library boundary and local links checked: 25 files.
make test
make[1]: Entering directory '/tmp/pde_assembly2_final_library'
...
make[1]: Leaving directory '/tmp/pde_assembly2_final_library'
...
test_input_and_range_contract (test_finite_reductions.FrozenQuadraticTests) ... ok
test_physical_half_loss_and_recomputed_raw_interpolation (test_finite_reductions.FrozenQuadraticTests) ... ok
test_raw_gradient_kernel_and_simultaneous_euler (test_finite_reductions.FrozenQuadraticTests) ... ok
test_zero_feature_zero_residual_and_return_ownership (test_finite_reductions.FrozenQuadraticTests) ... ok
