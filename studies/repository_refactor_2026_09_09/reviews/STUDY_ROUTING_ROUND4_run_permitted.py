...
if len(sys.argv) > 1:
    ...
    if os.environ.get('ACCEPTANCE_RETRY') == '1':
...
def run(item):
    index, relative = item
    prefix = 'retry' if os.environ.get('ACCEPTANCE_RETRY') == '1' else 'suite'
...
(PRIVATE / ('retry-results.json' if retry else 'test-results.json')).write_text(json.dumps(records, indent=2) + '\n')
