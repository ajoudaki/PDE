...
The present change substantially expands the actual
...
It leaves the eight proof chapters and established implementation unchanged.
...
This is an actual incorporation of the
narrative, **not** promotion of the remaining mathematical or code candidates.

Three next editorial assemblies have the highest value:
...
| 2. MFP computational machinery | Gaussian conditioning/response, finite-order flow identities, fixed-program width identification, rational Gaussian moments and finite dynamics APIs | Material omission. Assemble a small common jet/forest/certificate core; separately audit generator semantics, probability identification and displayed artifacts. Do not promote all historical compilers. |
| 3. Quantitative discretization and physical loss | Fixed-program width theorem; raw-GD/continuous-clock distinction | Material omission. Extend calculus with exact-compiler coefficients and explicit fixed-program remainders. Keep sharp shallow/linear bounds separate from general nonlinear growing-update claims. |
| 4. Stieltjes and formal Taylor | Same-space/jet-norm obstructions and the smooth-ODE correction; no Stieltjes generator/certificate or quadratic factorial-divergence proof | Worth incorporating at exact scope. Rational certificate extraction is a bounded candidate; positive-metric recurrence/probability and high-order generators require their own acceptance. Canonical all-order positivity is unresolved by the retained prefix. |
| 5. Shallow and broader linear capture | L3 one-input order-one-readout joint operator theorem and restricted nonclosure; different scalar-particle residual theorem | Distinct omissions. Assemble shallow characteristic, L2 spectral and every-fixed-depth linear GF proofs with exact clocks, initial laws and observables. No automatic arbitrary-data or raw-GD extension. |
| 6. Broader nonlinear and correlated-data progress | One-input shifted theorem; special-angle opposite-label L2; equal-label L3 at admitted correlations; three-sample bounded-shape/gain theorem; finite one-input pure-atan fitting | Closed generic-angle compactness and mixed finite-GF/gate partials deserve incorporation. Broad activation/local population and perturbation families retain specific import/ancestral-review qualifications, not a blanket rejection. |
| 7. Initialization geometry | Gain-based geometry and absolute-nonaffinity refinements inside the three-sample theorem | Partly included, but the odd-mixture, literal convex-offset and calibrated near-identity comparisons are materially absent. Consolidate them as initialization results, not trained-flow continuation. |
...
superseded. It does not contain forest compilation, a full finite-jet oracle,
canonical counting or the complete high-order coefficient engine.
...
identity. A future assembly should follow that exact route, not import a
stronger headline from a neighboring manuscript. The separate
...
contain useful algebra beyond the current clock warning. Their pullback words
...
offer a compact axis certificate with a direct expectation argument. This is a
diagnostic negative for a specified model/metric family, not a proposal to
freeze layers in the main learning theorem. Its current wrapper's historical
regression file/hash dependency must be separated from the mathematical core.
...
are genuinely different capture mechanisms. The current L3 theorem does not
prove them. Assemble the exact characteristic/rate, rooted spectral, and
two-endpoint operator arguments respectively. The last source's short
Wick-counting/sharp-norm inputs still need to be mapped into a contained proof.
No theorem should acquire a raw-GD limit merely because a neighboring linear
chapter has one, or an arbitrary dataset merely because depth is arbitrary.
...
should form one comparison, retaining each exact activation and normalization.
...
The current gain-based three-sample chapter covers none of these comparisons
merely by proving a different initialized Gram bound.
...
No coefficient production, numerical campaign, installation, native build,
seal reset or source-budget reuse was performed for this advisory. No historical
array or figure was promoted. Existing frozen campaigns remain frozen. The
current finite reference and rational Gaussian API are accepted at their own
scope; they must not be advertised as a population solver or full MFP compiler.
...
separate from those coverage recommendations. Exact reports and final acceptance
status are recorded in [VALIDATION.md](VALIDATION.md).
