...
| Gaussian/flow calculus | Finite two-sided conditioning, worked transpose empirical law, gradient identities, rational Wick arithmetic, fixed-depth/fixed-length feature-ascent width theorem and norm/jet obstructions | Growing-program or physical loss-GF convergence; complete high-order compiler tables |
...
| Special-data limits | Opposite-label L2 arctangent at orthogonal/antipodal inputs; equal-label shifted-arctangent L3 at all admitted correlations; three-sample bounded-shape/gain family | Generic-angle opposite-label arctangent, arbitrary sample count, or fitting in the special-angle L2 theorem |
...
| Finite optimization and controls | Canonical L3 arctangent finite GF/exact-GD fitting and endpoints; energy-compatible finite metric projection, integrated L1 defect and sufficiently large-cap exactness | Projection population/GD limit, RMS defect, or identification of all training endpoints as width grows |
| Reference implementation | Finite equal-width network APIs, layer-specific activations, batch loss/gradient/GF/GD/kernel formulas; exact rational Gaussian moments and PSD validation | Population simulator, full MFP compiler, arbitrary-precision floating network arithmetic or an empirical proof |
...
## Three distinct clipping statements
...
varying capped models. Combining the three statements does not fill that gap.
...
research: all-angle first-layer compactness; mixed-activation finite-GF fitting
and permanent first-gate mass; sharp initialization conditioning; given-space
capped flows; fixed-time time-doubling and shallow remainder estimates; finite
QI/IQ reductions. Each retains its original source and exact claim scope.
...
High-order MFP/Stieltjes tables and certificates remain with their producing
studies and retained data. Exact checks of a table are not proof that the
