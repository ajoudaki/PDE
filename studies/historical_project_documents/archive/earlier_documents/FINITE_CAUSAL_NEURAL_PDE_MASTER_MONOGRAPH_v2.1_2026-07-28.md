...
**Supersedes:** `FINITE_CAUSAL_NEURAL_PDE_MASTER_MONOGRAPH_v2.0_2026-07-28.md`  
**Project state:** 28 July 2026  
**Audit state:** A recovered 21 July primary artifact now supplies the quadratic derivative-history forest invariant, width power counting, and leading-contraction topology. It does not complete the fixed-order expectation, doubled-covariance, or \(L^1\) limit bridge `(FW)`, so every dependent zero-radius and compiler result remains conditional. The primary TP IV, TP VI, and self-consistent DMFT papers have also been checked directly and their precise theorem or claim status is incorporated below. Other unreproduced calculations and empirical artifacts remain explicit; see Appendix D.6–D.7.  
...
No new experiment is introduced in this chapter. All numerical values are inherited from the frozen reports and, where available, their later source-level audits. The complete raw arrays and executable code were not part of the supplied artifact bundle, so the empirical claims are source-audited reports, not independently reproduced computations.
...
5.  **Empirical reproduction:** the reports provide protocols and summaries, but the complete raw arrays and executable code were not supplied.
...
- **FAIL** for strict proof/reproduction completeness for exactly the six dependencies in §D.6.
...
Version 2.1 is the sole current master. It does not claim that the six
remaining nonlocal project dependencies have been repaired. Their exact
status and proof obligations are stated directly in §D.6; no separate
recovery guide is required. The empirical-reproduction item remains pending
the user's later supply of code, configurations, and raw arrays.
