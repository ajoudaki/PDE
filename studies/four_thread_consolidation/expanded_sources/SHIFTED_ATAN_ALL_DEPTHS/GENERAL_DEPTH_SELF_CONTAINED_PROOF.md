# One fixed nonlinear activation at every finite hidden depth

This document proves the complete joint mean-field/gradient-flow/exact-GD
theorem for every fixed integer number of hidden layers \(L\ge3\), using
the SAME activation \(\phi(z)=1+\arctan(z)/10\) at every layer and depth.
In particular it proves the four-hidden-layer case. Depth is fixed before
width tends to infinity. No simultaneous depth/width limit or estimate
uniform in depth for every observable is asserted.

The proof is self-contained apart from elementary probability, Gaussian
integration, basic measure theory, and Hilbert/Banach-space analysis.
No external mean-field, tensor-program, or continuation theorem is used.
Auxiliary clipping occurs only in the proof and is removed. The final
network and its dynamics have no clipping.

## 1. Exact model and theorem

There is one input equal to one and target one. Every hidden layer has
width \(n\). The first preactivation \(z^{(1)}\) and the RESCALED readout
\(W^{(L+1)}\) are vectors; \(W^{(\ell)}\), \(2\le\ell\le L\), are
\(n\) by \(n\) matrices. Define
\[
 \phi(z)=1+\arctan(z)/10,\qquad h^{(\ell)}=\phi(z^{(\ell)}),\qquad
 z^{(\ell)}=W^{(\ell)}h^{(\ell-1)}\quad(2\le\ell\le L),
\]
\[
 f_n=(W^{(L+1)})^Th^{(L)}/n,\quad r_n=f_n-1,\quad L_n=r_n^2.
\]
...
Theorem. For every fixed integer \(L\ge3\), there are \(L\) fixed neuron
probability spaces \(\Omega_\ell\) and bounded initial Gaussian matrix
actions between adjacent \(L^2\) spaces, with their actual adjoints, on
which the \(L+1\) present objects
\[
 X^{(1)},\quad W^{(2)},\ldots,W^{(L)},\quad W^{(L+1)},
 \qquad Z^{(1)}=F^{-1}(X^{(1)}),\quad F(z)=10(z+z^3/3) \tag{1.4}
\]
define an autonomous population gradient flow. Initial \(Z^{(1)}\) is
standard Gaussian and the initial population readout is zero.
Forward and backward equations are (1.1) with capital \(Z,H\) and
population adjoints \(^{*}\) replacing finite transposes \(^{T}\).
Expectations always refer to the population containing the integrand.
For adjacent fields write
\((U\otimes V)B=U\mathbb E[VB]\). The equations are
\[
 f=\mathbb E[W^{(L+1)}H^{(L)}],\quad r=f-1,\quad
 \dot X^{(1)}=-2r q^{(1)},\quad
...
Here \(s\) is feature time, \(\Delta\) its Euler step; primes on paths
...
and every interior \(|\delta_R^{(\ell)}_k|\le R/5\).
...
All oracle norms are bounded in probability. Thus the differences
vanish. This is a FIXED-program result, not a mesh-uniform induction.

The exact resulting scalar law, omitting \(R\) on its backwards fields,
is
\[
 X^{(1)}_k=F(Z^{(1)}_0)+\Delta\sum_{r<k}q^{(1)}_r,\quad
 H^{(1)}_k=\chi(X^{(1)}_k),
\]
\[
 Z^{(\ell)}_k=\xi^{(\ell)}_k+
            \sum_{r<k}a^{(\ell)}_{kr}\delta^{(\ell)}_r,\quad
 H^{(\ell)}_k=\phi(Z^{(\ell)}_k)\quad(2\le\ell\le L),
\]
\[
 W^{(L+1)}_k=\Delta\sum_{r<k}H^{(L)}_r,\quad
...
\[
 \mathbb E[\xi^{(\ell)}_k\xi^{(\ell)}_r]
       =\mathbb E[H^{(\ell-1)}_kH^{(\ell-1)}_r],\quad
 \mathbb E[\zeta^{(\ell-1)}_k\zeta^{(\ell-1)}_r]
       =\mathbb E[\delta^{(\ell)}_k\delta^{(\ell)}_r].
                                                               \tag{4.4}
\]
...
\[
 ...
 \mathbb E\frac{\partial H^{(\ell-1)}_k}
                  {\partial\zeta^{(\ell-1)}_r}
       +\Delta\mathbb E[H^{(\ell-1)}_kH^{(\ell-1)}_r]\quad(r<k),
\]
\[
 b^{(\ell)}_{kr}=
 \mathbb E\frac{\partial\delta^{(\ell)}_k}{\partial\xi^{(\ell)}_r}
       +\Delta\mathbf1_{r<k}
                   \mathbb E[\delta^{(\ell)}_k\delta^{(\ell)}_r]
                                                     \quad(r\le k).
                                                               \tag{4.5}
...
\[
 ...
 \sum_{\ell=2}^L\|W^{(\ell)}-\widetilde W^{(\ell)}\|_{\rm op}
 +\|W^{(L+1)}-\widetilde W^{(L+1)}\|_2.                \tag{5.2}
\]
...
at most \(S\), initial readout norm at most one and initial matrix norm
at most \(M_0\). Set
\[
...
\]
These bound the readout, backwards fields, and matrix norms by
\(Q_0,D_\ell,R_\ell\), respectively: integrate or sum the bounded
readout velocity, then use
\(\|\delta_R^{(\ell)}\|_2\le\|q_R^{(\ell)}\|_2/10\)
and the rank-one norm identity \(\|U\otimes V\|=\|U\|_2\|V\|_2\)
successively down the layers. Finally
\(\|q_R^{(1)}\|_2\le R_2D_2\) bounds the first-coordinate velocity.
...
\[
 ...
 \le a\|W_A^{(2)}-W_B^{(2)}\|_{\rm op}
       +(R_2/100)\|X_A^{(1)}-X_B^{(1)}\|_2,
\]
\[
 \|Z_A^{(\ell)}-Z_B^{(\ell)}\|_2
 \le a\|W_A^{(\ell)}-W_B^{(\ell)}\|_{\rm op}
       +(R_\ell/10)\|Z_A^{(\ell-1)}-Z_B^{(\ell-1)}\|_2.
                                                               \tag{5.6}
\]
...
\[
 \|\delta_A^{(L)}-\delta_B^{(L)}\|_2
 \le \tfrac1{10}\|W_A^{(L+1)}-W_B^{(L+1)}\|_2
                  +\tfrac{B_0}{5}\|Z_A^{(L)}-Z_B^{(L)}\|_2.
                                                               \tag{5.7}
\]
...
\[
 \|q_A^{(\ell)}-q_B^{(\ell)}\|_2
 \le R_{\ell+1}\|\delta_A^{(\ell+1)}-\delta_B^{(\ell+1)}\|_2
             +D_{\ell+1}\|W_A^{(\ell+1)}-W_B^{(\ell+1)}\|_{\rm op}.
\]
At a common cap,
\[
 \|\delta_R^{(\ell)}(A)-\delta_R^{(\ell)}(B)\|_2
 \le\tfrac1{10}\|q_A^{(\ell)}-q_B^{(\ell)}\|_2
             +\tfrac{2R}{5}\|Z_A^{(\ell)}-Z_B^{(\ell)}\|_2.
                                                               \tag{5.8}
\]
...
bounds \(B_{\ell,r}\le1\), \(\|\delta^{(\ell)}_r\|_2\le7/40\).
The following forward pass constructs all current forward coefficients
using only those past bounds.

For a single bottom source \(\zeta^{(1)}_r\), differentiating the
complete bottom expression gives
\[
 |\partial_{\zeta^{(1)}_r}X^{(1)}_j|
 \le\Delta\mathbf1_{r<j}
 +\frac{\Delta}{100}\sum_{u<j}\sum_{v\le u}|b^{(2)}_{uv}|
                    |\partial_{\zeta^{(1)}_r}X^{(1)}_v|.
\]
By (2.5), for \(r<j\le k\),
\[
 |\partial_{\zeta^{(1)}_r}H^{(1)}_j|
...
has established \(|a^{(\ell)}_{jr}|\le A\Delta\) for \(j\le k\).
Define its forward derivative row and its nondecreasing envelope by
\[
 \mathcal R_{\ell,j}
 =\sum_{r\le j}|\partial_{\xi^{(\ell)}_r}Z^{(\ell)}_j|,
 \quad
...
For a SINGLE reverse source \(\zeta^{(\ell)}_r\), the extra derivative
of \(q^{(\ell)}_u\) is \(\mathbf1_{u=r}\).
Its first forward contribution has size at most \(A\Delta/10\).
The same comparison yields
\[
 |\partial_{\zeta^{(\ell)}_r}H^{(\ell)}_j|
       \le(A\Delta/100)E_{\ell,j}\quad(r<j\le k).       \tag{6.4}
\]
...
\[
 \|E_{\ell,j}\|_1<4,\quad\|E_{\ell,j}\|_2<3,\qquad
 |a^{(\ell+1)}_{jr}|
 \le\Delta(a^2+4A/100)
...
Let \(T_j=\sum_{r\le j}|\partial_{\xi^{(L)}_r}Z^{(L)}_j|\).
Differentiating the readout sum and top gate gives
\[
 \sum_{r\le j}|\partial_{\xi^{(L)}_r}\delta^{(L)}_j|
...
\[
 ...
 +\sum_{\ell=2}^L
       \|\delta_{R'}^{(\ell)}(A)-\delta_R^{(\ell)}(B)\|_2
 +\sum_{\ell=1}^{L-1}\|q_{R'}^{(\ell)}(A)-q_R^{(\ell)}(B)\|_2
 \le C(1+R)d(A,B)
        +C\sum_{\ell=2}^{L-1}\|b_R(q_R^{(\ell)}(B))\|_2.           \tag{7.3}
\]
...
one adds \(C R\,d(A,B)\) and \(C\|b_R(q_B^{(\ell)})\|_2\).
...
\[
 ...
 \quad
 a_R(s)=\sum_{\ell=2}^{L-1}\|b_R(q_R^{(\ell)}(s))\|_2.
\]
...
\(\|A\|_{\rm HS}^2=\sum_j\|Ae_j\|_2^2\). Parseval proves independence
of the basis and \(\|U\otimes V\|_{\rm HS}=\|U\|_2\|V\|_2\).
...
let \(\|v\|_2\to0\), then remove \(R\); the remainder is \(o(\|v\|_2)\).
...
\[
 df=\mathbb E[\delta^{(1)}dZ^{(1)}]
   +\sum_{\ell=2}^L\mathbb E[\delta^{(\ell)}dW^{(\ell)}H^{(\ell-1)}]
   +\mathbb E[H^{(L)}dW^{(L+1)}].
\]
The identity
...
\[
 ...
 K^{(\ell)}=\mathbb E[(\delta^{(\ell)})^2]
             \mathbb E[(H^{(\ell-1)})^2]\ (2\le\ell\le L),
 \quad K^{(L+1)}=\mathbb E[(H^{(L)})^2]\ge25/36.        \tag{9.4}
\]
...
\[
 ...
 \quad L_t=-4(f-1)^2\sum_\ell K^{(\ell)}.              \tag{9.6}
\]
...
norm plus \(2a|r_n(0)|t\). Integrating the matrix norm bounds successively from layer \(L\)
...
\[
 ...
 (Z^{(\ell)})'=\mathbb E[(H^{(\ell-1)})^2]\delta^{(\ell)}
...
Write \(m_0=1\), \(m_\ell=\mathbb E[(H^{(\ell)}_0)^2]\).
...
Put \(\sigma_\ell^2=\mathbb E[(B^{(\ell)})^2]\) and
\[
 \kappa_\ell=\frac1{100m_{\ell-1}}
     \mathbb E\frac{Z^{(\ell)}_0\arctan Z^{(\ell)}_0}
                     {1+(Z^{(\ell)}_0)^2}>0\quad(2\le\ell\le L).
\]
Start with \(c_L=\kappa_L\) and \(\sigma_L^2>0\).
...
\[
 c_\ell=\kappa_\ell c_{\ell+1}>0,\quad
 \sigma_\ell^2=
...
\(\mathbb E[Z^{(L)}_0B^{(L)}]/m_{L-1}=\kappa_L\):
...
Gaussian symmetry gives \(c_\ell=\kappa_\ell c_{\ell+1}\).
...
has vanishing normalized squared norm, and conditional Gaussian
...
\[
 ...
 K^{(L+1)}(s)=m_L+\Gamma s^2+o(s^2),\quad
...
\[
 ...
 K^{(L+1)}(t)=m_L+4\Gamma t^2+o(t^2),
\]
\[
 \sum_{\ell=1}^{L+1}K^{(\ell)}(t)
                         =m_L+8\Gamma t^2+o(t^2).    \tag{12.7}
\]
...
Each forward source variance lies between \(m^2\) and \(a^2\).
At the top, Section 6 implies
\[
 |Z^{(L)}_k-\xi^{(L)}_k|\le AaS^2/10=63/160,\quad
 \mathbb P(\pm Z^{(L)}_k\ge u)
       \ge\overline\Phi((u+63/160)/m)>0.              \tag{12.8}
\]
...
\[
 ...
 \ge\frac{1117}{1600}\overline\Phi((u+1)/m)>0.          \tag{12.9}
\]
...
\(W^{(L+1)}(s)\ge ms\) almost surely. Since \(\phi'>0\),
