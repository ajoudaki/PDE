# An odd convex activation under absolute angular separation
...
\]
All hidden kernel blocks initially vanish because \(C(A.0)=0\). Hence
\[
\kappa_{a,e}(A.0)=\tfrac14y^TK^4(A.0)y
=\frac{q_3+\tau c_3}{2}
...
\kappa_{a,0}(A.0)=a^6(1+\tau\rho)/2. \tag{A.2}
\]
...
depend only on \((P_1,A,B,C)\), never on the inactive root \(Q_1(A.0)\).
...
Its initial right slope is \(\|H(A.0)\|\), because
\(C(A.0)=0,C'(A.0)=H(A.0)\). Thus \(\|C'(s)\|\ge\|H(A.0)\|\) and
\[
g'\ge\kappa_0,\qquad
