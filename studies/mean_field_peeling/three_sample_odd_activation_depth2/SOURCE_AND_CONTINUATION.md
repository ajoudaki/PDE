...
1. A bounded raw state ball has no uniform incoming-field tail estimate. Hold w,A at initialization and let C range over an L^2 ball in the top canonical space. The readout may have an arbitrary L^2 tail; in particular the squared magnitudes in a bounded L^2 ball need not be uniformly integrable: there is no common vanishing L^2 tail bound or common positive exponential moment. The source lemma needs a reachable-state estimate, not a bound for an arbitrary ambient ball.
