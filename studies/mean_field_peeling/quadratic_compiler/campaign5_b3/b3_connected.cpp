  ...
  Poly value(const Tree &t, int k) {
    ...
    for (size_t u = 0; u < t.a.size(); ++u) if (t.a[u])
      for (int alpha = 0; alpha < 3; ++alpha) {
        ...
        add_shift(ans, value(child, k - 1), Integer(multiplicity));
      }
...
int main(int argc, char **argv) {
  try {
    if (argc != 2)
      throw std::invalid_argument("usage: b3_connected ORDER");
    int max_order = std::stoi(argv[1]);
    if (max_order < 0 || max_order > 5)
      throw std::invalid_argument("ORDER must be between zero and five");

    Recursion recursion;
    std::cout << "{\"colors\":3,\"raw_rho\":[";
    for (int k = 0; k <= max_order; ++k) {
      auto start = std::chrono::steady_clock::now();
      Poly result{0};
      add_shift(result, recursion.value(root_for_alpha(0), k), 1);
      add_shift(result, recursion.value(root_for_alpha(1), k), 1);
