// Exact connected-recursion compiler for Campaign 5.
//
// The raw state is a connected bipartite tree.  Row vertices carry powers of
// the readout Gaussian a. Column vertices carry exponent triples for the
// three equicorrelated input colors. Coefficients are exact signed polynomials
// in rho. The same Gram G(rho) is used in initialization moments and the
// first-layer gradient rewrite.
...
using Poly = std::vector<Integer>;  // coefficient of rho^r

struct Tree {
  std::vector<unsigned char> a;
  std::vector<std::array<unsigned char, 3>> h;
...
static Poly equicorrelated_moment(const std::array<int, 3> &powers) {
  int total = powers[0] + powers[1] + powers[2];
  if (total & 1) return Poly{0};
  std::vector<unsigned char> labels;
  for (int color = 0; color < 3; ++color)
    for (int q = 0; q < powers[color]; ++q)
      labels.push_back(static_cast<unsigned char>(color));
  Poly answer(static_cast<size_t>(total / 2 + 1), 0);
  auto pair = [&](auto &&self, std::vector<unsigned char> rest,
                  int cross_pairs) -> void {
    if (rest.empty()) {
      ++answer[cross_pairs];
      return;
    }
    unsigned char first = rest.front();
    for (size_t position = 1; position < rest.size(); ++position) {
      unsigned char second = rest[position];
      auto child = rest;
      child.erase(child.begin() + static_cast<long>(position));
      child.erase(child.begin());
      self(self, std::move(child), cross_pairs + (first != second));
    }
  };
  pair(pair, std::move(labels), 0);
  trim(answer);
  return answer;
}
...
static std::string rooted_code(
    ...
    const std::array<int, 3> &color_permutation) {
  ...
  if (vertex < rows) {
    ...
    out.push_back('H');
    out.push_back(static_cast<char>(1 + z[color_permutation[0]]));
    out.push_back(static_cast<char>(1 + z[color_permutation[1]]));
    out.push_back(static_cast<char>(1 + z[color_permutation[2]]));
  }
...
// partitions directly.  This is the three-color analogue of the checked
// parent compiler's VertexPartitionWickEvaluator.
struct VertexPartitionEquicorrelated {
  const Tree &tree;
  int rows, cols, pairs, target;
  std::vector<std::vector<int>> incidence;

  explicit VertexPartitionEquicorrelated(const Tree &t)
  ...
  struct ColumnDP {
    ...
    Poly rec(const std::vector<int> &remaining, int blocks) {
      ...
      std::array<int, 3> power_sum = types[distinguished].powers;
      ...
      auto enumerate = [&](auto &&self, int type_index,
                           std::array<int, 3> current_powers) -> void {
        if (type_index == static_cast<int>(types.size())) {
          ...
          Poly product = multiply(equicorrelated_moment(current_powers),
                                  rec(child, blocks - 1));
        ...
        for (int extra = 0; extra <= maximum; ++extra) {
          ...
          auto following_powers = current_powers;
          for (int color = 0; color < 3; ++color)
            following_powers[color] +=
                extra * types[type_index].powers[color];
          self(self, type_index + 1, following_powers);
          for (int q = 0; q < row_blocks; ++q)
            edge_sum[q] -= extra * types[type_index].signature[q];
          take[type_index] -= extra;
        }
      };
      enumerate(enumerate, 0, power_sum);
  ...
  Poly run() const {
    ...
    if (tree.edges.empty()) {
      ...
      if (rows == 0 && cols == 1)
        return equicorrelated_moment(
            {tree.h[0][0], tree.h[0][1], tree.h[0][2]});
      return Poly{0};
    }
...
// leaf is a polynomial because each surviving column class has a
// three-color equicorrelated Gaussian moment.
struct WickEvaluator {
  ...
  static void merge_cols(
      int x, int y, std::vector<std::array<unsigned char, 3>> &h,
      std::vector<std::vector<unsigned char>> &m,
      std::vector<std::vector<unsigned char>> &sealed) {
    if (x == y) return;
    if (x > y) std::swap(x, y);
  ...
  std::string key_of(
      ...
      const std::vector<std::array<unsigned char, 3>> &h,
    ...
    for (auto z : h) {
      key.push_back(static_cast<char>(z[0]));
      key.push_back(static_cast<char>(z[1]));
      key.push_back(static_cast<char>(z[2]));
    }
  ...
  Poly leaf(const std::vector<unsigned char> &a,
            const std::vector<std::array<unsigned char, 3>> &h) const {
    ...
    for (auto z : h)
      ans = multiply(ans, equicorrelated_moment({z[0], z[1], z[2]}));
    return ans;
  }
...
struct Recursion {
  ...
  std::string key(const Tree &t, int k, int w_hits) const {
    std::string out;
    out.push_back(static_cast<char>(k));
    out.push_back(static_cast<char>(w_hits + 1));
    out += canonical_key(t);
    return out;
  }
  ...
  Poly value(const Tree &t, int k, int w_hits) {
    ...
    for (size_t u = 0; u < t.a.size(); ++u) if (t.a[u])
      for (int alpha = 0; alpha < 3; ++alpha) {
        ...
        --child.a[u];
        std::array<unsigned char, 3> exponent{0, 0, 0};
        exponent[alpha] = 2;
        ...
        add_shift(ans, value(child, k - 1, w_hits), Integer(multiplicity));
      }

    // Dtilde u rewrite, including Q_beta,alpha.
    for (size_t v = 0; v < t.h.size(); ++v)
      for (int beta = 0; beta < 3; ++beta) if (t.h[v][beta])
        for (int alpha = 0; alpha < 3; ++alpha) {
          Tree child = t;
          ...
          add_shift(ans, value(child, k - 1, w_hits),
                    Integer(multiplicity), alpha != beta);
        }
    ...
    for (size_t edge = 0; edge < t.edges.size(); ++edge)
      for (int alpha = 0; alpha < 3; ++alpha) {
        ...
        Poly convolution{0};
        for (int q = 0; q <= k - 1; ++q) {
          for (int left_hits = 0; left_hits <= w_hits - 1; ++left_hits) {
            Poly product = multiply(
                value(children[0], q, left_hits),
                value(children[1], k - 1 - q,
                      w_hits - 1 - left_hits));
            add_shift(convolution, product, choose(k - 1, q));
          }
        }
        add_shift(ans, convolution, Integer(2));
      }
  ...
  try {
    if (argc != 4)
      throw std::invalid_argument(
          "usage: stage_c_sector ORDER --w-hits COUNT");
    int max_order = std::stoi(argv[1]);
    if ((max_order != 5 && max_order != 7) ||
        std::string(argv[2]) != "--w-hits")
      throw std::invalid_argument(
          "sector compiler requires ORDER=5 or ORDER=7 and --w-hits");
    int required_w_hits = std::stoi(argv[3]);
    if (required_w_hits < 0 || required_w_hits > max_order)
      throw std::invalid_argument("W-hit sector is outside the order");

    Recursion recursion;
    std::cout << "{\"colors\":3,\"w_hits\":" << required_w_hits
              << ",\"raw_rho\":[";
    for (int k = 0; k <= max_order; ++k) {
      ...
      if (k != max_order) {
        ...
        add_shift(result, recursion.value(root_for_alpha(0), k,
                                          required_w_hits), 1);
        add_shift(result, recursion.value(root_for_alpha(1), k,
                                          required_w_hits), 1);
        add_shift(result, recursion.value(root_for_alpha(2), k,
                                          required_w_hits), 1);
      }
    ...
    std::cout << "],\"normalization\":\"divide order k by 3^(k+1)\"," 
              << "\"value_cache\":" << recursion.value_memo.size()
              << ",\"wick_cache\":" << recursion.wick_memo.size()
