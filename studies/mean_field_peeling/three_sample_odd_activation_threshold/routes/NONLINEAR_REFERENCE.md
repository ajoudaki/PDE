...
A sufficient proof cannot stop at the sharp `e^2 delta^2` initialization margin. One viable uniform-clock nonlinear-reference route would supply all of the following, with constants independent of width, cutoff and physical horizon. This is a sufficient route, not a necessity claim: a direct compact-physical-time construction may instead have constants depending on T, provided the activation choice is independent of T.
