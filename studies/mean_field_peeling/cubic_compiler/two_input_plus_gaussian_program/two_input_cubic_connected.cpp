// Exact connected-recursion compiler for the two-input raw-cubic plus jet.
//
  ...
  try {
    if (argc < 3 || argc > 4)
      throw std::invalid_argument(
          "usage: two_input_cubic_connected SIGN ORDER [audit]");
    ...
    bool audit_terminal = argc == 4 && std::string(argv[3]) == "audit";
    if (argc == 4 && !audit_terminal)
      throw std::invalid_argument("optional fourth argument must be audit");
