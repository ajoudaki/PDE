...
New derivation 2026-09-08. This extends `SHAPE_SYMMETRY.md`. It replaces boundedness of psi by bounded first two derivatives plus control at zero, and gives a direct raw-moment proof avoiding normalization by a small inactive variance. It retains the mathematical source dependencies listed in that file; no prior review outcome is a premise.
...
The old raw radius-one tube also bounds the normalized active first root p by b: its norm change is controlled by raw first-layer displacement. This bound is stronger than a bound on the unnormalized sample projections and is used in the next affine prediction estimate.
