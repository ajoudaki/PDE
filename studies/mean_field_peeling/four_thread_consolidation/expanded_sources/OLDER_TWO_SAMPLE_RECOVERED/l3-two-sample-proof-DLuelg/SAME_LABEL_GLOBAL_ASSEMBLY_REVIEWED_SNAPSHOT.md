...
Root working proof, 2026-09-06. NOT YET AUDITED. This is a modular
...
using its fixed-cap Lipschitz field and the bounded smooth population
clock. Consequently the maximal stopped node discrepancy is at most
...
The analytical construction/comparison assembly above needs a complete
independent audit together with its dependencies. Strict distributional
nonlinearity and every-layer nonfreezing still require a separate
two-sample proof; the one-sample positivity argument cannot simply be
quoted for potentially cancelling sample gradients.
