...
spaces are separate copies of \(\mathbb R^n\), each with
\[
 \langle v,w\rangle_n=\frac1n v^Tw,\qquad
 \|v\|_n^2=\frac1n\sum_i|v_i|^2.
 \tag{8}
\]
...
probe is formed by a finite program from the initialization, independent
...
normalized inner products, actions of \(W^{(2)}_n(0)\) or its transpose,
and the learned rank integrals at prescribed times. Limits of such
...
Every original hidden-field velocity has joint Wasserstein-2 convergence
...
are strictly positive in their stated norms. The hidden speeds at zero
are zero. The full kernel is nonconstant on every sufficiently short
...
operator, and readout bounds, \(\|q\|_n\le C_T\) and
...
\[
 \sup_{t\le T}\left(\sum_a\|u^{\rm GD}_{n,a}-u^{\rm GF}_{n,a}\|_n
   +\|W^{(2),\rm GD}_n-W^{(2),\rm GF}_n\|_F
   +\|W^{(3),\rm GD}_n-W^{(3),\rm GF}_n\|_n\right)
                          =O_T(n^{-3/2}).                    \tag{31}
\]
...
\(K_R=1+\sum_j\|R_j\|_n\), and write the initial matrix as
...
\[
 \begin{aligned}
 \|x\|_n+|c|&\le P(K_R+\|W^{(2)}_n(0)\|_{\mathrm{op}}),\\
...
Euclidean norm at most \(\|v\|_F\|x\|_n\). For a contraction,
\[
 |\partial\langle x,y\rangle_n[v]|
 \le n^{-1/2}(\|y\|_n\|\partial x[v]\|_{\ell^2}
                     +\|x\|_n\|\partial y[v]\|_{\ell^2}).
\]
...
include its proof. For a smooth scalar function \(X(E)\),
\[
...
\]
Also \(\|m(R)\|_n\le P(K_R)\). Average over \(j\) and use
...
has growth \(C(1+|x|^d)\), its empirical conditional variance, on a
bounded regression-coefficient event, is at most
\[
 \frac Cn\left(1+\frac1n\sum_i|\text{old tuple}_i|^{2d}\right).
 \tag{48}
\]
Conditional Chebyshev proves concentration. Its conditional expectation
...
\[
 |\langle x,y\rangle_n-\langle x',y'\rangle_n|
 \le\|x-x'\|_n\|y\|_n+\|x'\|_n\|y-y'\|_n,
 \tag{52}
\]
gives \(\|x_n^\epsilon-x_n^0\|_n\le\epsilon P(K_n)\), where
...
\[
 ...
 W^{(2)}_kh&=W^{(2)}_0h+
            \sum_{r<k,b}\gamma_{rb}\delta^{(2)}_{rb}
                                          \langle H^{(1)}_{rb},h\rangle,\\
 W^{(2)*}_kv&=W^{(2)*}_0v+
            \sum_{r<k,b}\gamma_{rb}H^{(1)}_{rb}
                                          \langle\delta^{(2)}_{rb},v\rangle.
 \end{aligned}                                                   \tag{54}
\]
...
\[
 ...
 \quad
 \langle a(h),v\rangle_2=\langle h,b(v)\rangle_1.            \tag{55}
\]
...
immediately, by RMS at most \(C_T h_s|\epsilon|\|e\|_n\).
Multiplying the later factors \(1+C_T h_k\) gives, for \(k>s\),
\[
 \|h^{(1),\epsilon}_{n,ka}-h^{(1),0}_{n,ka}\|_n
                 \le C_T h_s|\epsilon|\|e\|_n.              \tag{58}
\]
...
are at most \(C_T|\epsilon|\|e\|_n\). The actual transpose bounds
the immediate reverse-answer changes by the same quantity. Every
state update has its factor \(h_s\), so at later nodes
\[
 \|\delta^{(2),\epsilon}_{n,ka}-\delta^{(2),0}_{n,ka}\|_n
                 \le C_T h_s|\epsilon|\|e\|_n\qquad(k>s).
 \tag{59}
\]
At the current node the delta change is at most
\(M_T|\epsilon|\|e\|_n\), and other current sample deltas have
...
\(|\langle e,X_n^\epsilon-X_n^0\rangle_n|
\le\|e\|_n\|X_n^\epsilon-X_n^0\|_n\) to the joint limit in
...
\(C_T\|W^{(3)}_n(0)\|_n\to0\) in probability. Product estimates
...
\[
 ...
 \tag{84}
\]
By (80), \(r=(f_1-1)y\). Equation (24) implies
\[
