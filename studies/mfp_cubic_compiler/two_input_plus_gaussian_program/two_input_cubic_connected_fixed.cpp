// Fixed-correlation connected compiler for the two-input raw-cubic plus jet.
//
...
// rho.  The two-input covariance Q(rho) is used both in the terminal
...
enum class RhoMode { zero, half, one };
static RhoMode rho_mode = RhoMode::zero;

struct Dyadic {
  Integer numerator;
  unsigned denominator_power;

  Dyadic(Integer value = 0, unsigned power = 0)
      : numerator(std::move(value)), denominator_power(power) {
    normalize();
  }
  ...
  void normalize() {
    ...
    while (denominator_power && numerator % 2 == 0) {
      numerator /= 2;
      --denominator_power;
    }
  }
  ...
  bool operator==(const Dyadic &other) const {
    return numerator == other.numerator &&
           denominator_power == other.denominator_power;
  }
...
static void add_shift(Poly &out, const Poly &p, const Integer &scale,
  ...
  if (rho_mode == RhoMode::zero && shift) return;
  unsigned term_power = p.denominator_power;
  if (rho_mode == RhoMode::half)
    term_power += static_cast<unsigned>(shift);
  unsigned common = std::max(out.denominator_power, term_power);
  Integer left = out.numerator;
  Integer right = p.numerator * scale;
  left <<= common - out.denominator_power;
  right <<= common - term_power;
  out.numerator = left + right;
  out.denominator_power = common;
  trim(out);
}
...
static Poly multiply(const Poly &p, const Poly &q) {
  ...
  return Poly{
      ...
      p.denominator_power + q.denominator_power,
  };
}
...
static Poly bivariate_moment(int p, int q) {
  ...
  for (int r = 0; r <= std::min(p, q); ++r) {
    ...
    add_shift(ans, Poly{coefficient}, 1, r);
  }
  trim(ans);
  return ans;
}
...
static Tree exchange_colors(Tree t) {
  for (auto &exponents : t.h)
    std::swap(exponents[0], exponents[1]);
  return t;
}
...
struct WickEvaluator {
  ...
  Poly leaf(const std::vector<unsigned char> &a,
    ...
    for (auto z : a) {
      ...
      ans.numerator *= odd_df(static_cast<int>(z) - 1);
      ans.normalize();
    }
...
struct Recursion {
  ...
  explicit Recursion(int sign, bool audit)
      : sigma(sign), audit_terminal(audit) {}

  std::string symmetry_key(const Tree &t) const {
    std::string direct = canonical_key(t);
    if (sigma != 1) return direct;
    std::string exchanged = canonical_key(exchange_colors(t));
    return std::min(direct, exchanged);
  }
  ...
  Poly base(const Tree &t) {
    std::string k = symmetry_key(t);
    auto found = wick_memo.find(k);
    if (found != wick_memo.end()) return found->second;
    Poly ans = VertexPartitionBivariate(t).run();
    if (audit_terminal) {
      Poly reference = WickEvaluator().run(t);
      if (ans != reference)
        throw std::logic_error("terminal evaluators disagree");
    }
  ...
  Poly value(const Tree &t, int k) {
    int a_power = std::accumulate(t.a.begin(), t.a.end(), 0);
    if ((static_cast<int>(t.edges.size()) + k) & 1) return Poly{0};
    if ((a_power + k) & 1) return Poly{0};
    ...
    // Cubic Dtilde a rewrite: replace a by z_alpha^3.
      ...
      for (int beta = 0; beta < 2; ++beta) if (t.h[v][beta])
        for (int alpha = 0; alpha < 2; ++alpha) {
          ...
          child.edges.push_back(
              {u0, static_cast<unsigned char>(v1 + 1)});
          add_shift(ans, value(child, k - 1),
                    Integer(multiplicity) * sign[alpha], alpha != beta);
        }

    // Cubic Dtilde W rewrite.  The removed bridge splits the raw tree.
    for (size_t edge = 0; edge < t.edges.size(); ++edge)
      for (int alpha = 0; alpha < 2; ++alpha) {
        ...
        forest.edges.push_back(
            {u, static_cast<unsigned char>(fresh + 1)});
        ...
        add_shift(ans, convolution, Integer(3) * sign[alpha]);
      }
...
static Tree root_for_alpha(int alpha) {
  ...
  std::array<unsigned char, 2> exponent{0, 0};
  exponent[alpha] = 3;
  root.h = {exponent, exponent, exponent};
  root.edges = {{0, 0}, {0, 1}, {0, 2}};
  return root;
}
...
static std::string poly_json(const Poly &p) {
  ...
  out << '"' << p.numerator;
  if (p.denominator_power)
    out << '/' << (Integer(1) << p.denominator_power);
  out << '"';
  return out.str();
}
  ...
  try {
    if (argc < 4 || argc > 5)
      throw std::invalid_argument(
          "usage: two_input_cubic_connected_fixed SIGN ORDER RHO [audit]");
    ...
    if (max_order < 0 || max_order > 9)
      throw std::invalid_argument("ORDER must be between zero and nine");
    std::string rho = argv[3];
    if (rho == "0") {
      rho_mode = RhoMode::zero;
    } else if (rho == "1/2") {
      rho_mode = RhoMode::half;
    } else if (rho == "1") {
      rho_mode = RhoMode::one;
    } else {
      throw std::invalid_argument("RHO must be 0, 1/2, or 1");
    }
    bool audit_terminal = argc == 5 && std::string(argv[4]) == "audit";
    if (argc == 5 && !audit_terminal)
      throw std::invalid_argument("optional fifth argument must be audit");

    Recursion recursion(sigma, audit_terminal);
    std::cout << "{\"channel\":\"" << channel << "\",\"rho\":\""
              << rho << "\",\"raw_values\":[";
    for (int k = 0; k <= max_order; ++k) {
      auto start = std::chrono::steady_clock::now();
      Poly result{0};
      if (sigma == 1) {
        add_shift(result, recursion.value(root_for_alpha(0), k), 2);
      } else {
        add_shift(result, recursion.value(root_for_alpha(0), k), 1);
        add_shift(
            result, recursion.value(root_for_alpha(1), k), Integer(sigma));
      }
    ...
    std::cout << "],\"activation\":\"raw cubic\","
              << "\"terminal_audit\":" << (audit_terminal ? "true" : "false")
              << ",\"normalization\":\"divide order k by 2^(k+1)\","
              << "\"value_cache\":" << recursion.value_memo.size()
              << ",\"wick_cache\":" << recursion.wick_memo.size()
