...
Section C.4 separately proves local training-law stability and an
...
These fragments extend the global nonlinear and special-data chapters. Except for C.4, their probability statements take width to infinity at a fixed depth, fixed finite dataset, fixed activation, and fixed finite physical horizon. C.4 proves its stated simultaneous sampling/width/GD-step limit for the fixed two-hidden-tanh circle-input model on a common local interval. Constants uniform over a parameter class do not by themselves assert a probability supremum over that class. Layer populations are separate. Initialized actions and their adjoints are constructed jointly; only trained increments are Hilbert--Schmidt.
...
the theorem. No cross-width or finite-to-population operator
...
for any reference oracle field p and gate g=tanh',
...
For m>=L_Z insert `q<=L_Z/m` into the nondecreasing enlarged modulus
(or use its unspecialized cutoff estimate with q<=L_Z/m). Taking
`R=K sqrt(log(em))` gives (A11) directly. Finitely many smaller m are
...
Use the common fields (T1) from Section C.4.2. Write
