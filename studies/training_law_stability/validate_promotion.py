"""Validate/assemble the frozen proof-only P1 edition without a repository import."""
import argparse
import hashlib
import json
...
def main():
    ...
    manifest = json.loads((inputs / 'P1_MANIFEST.json').read_text())
    checks = []
    ...
    baseline = (inputs / 'P1_GLOBAL_BASELINE.md').read_text()
    assembled = baseline
    for edit in json.loads((inputs / 'P1_GLOBAL_EDITS.json').read_text()):
        assert assembled.count(edit['old']) == 1, edit['old'][:60]
        assembled = assembled.replace(edit['old'], edit['new'])
    addition = (inputs / 'P1_ADDITION.md').read_text()
    assembled = assembled.rstrip() + '\n\n' + addition
    assert assembled == (inputs / 'P1_GLOBAL_EDITION.md').read_text()
    ...
    for forbidden in ['studies/', 'data/generated/', '/root', 'R1_', 'P1_']:
        assert forbidden not in addition, forbidden
    for text in [addition, (inputs / 'P1_DEPENDENCIES.md').read_text()]:
    ...
    dep = (inputs / 'P1_DEPENDENCIES.md').read_text()
    excerpts = {}
    ...
    (docs / 'README.md').write_bytes((inputs / 'P1_DOCS_README.md').read_bytes())
    ...
    for target in re.findall(r'\]\(([^)]+)\)', addition):
        check_link(target)
    new_guide = (inputs / 'P1_DOCS_README.md').read_text()
    old_guide = (inputs / 'P1_README_BASELINE.md').read_text()
