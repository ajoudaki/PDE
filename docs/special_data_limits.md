# Population limits and correlated-data geometry

This chapter proves three distinct global nonlinear dense-network families,
a separate strong first-layer compactness theorem at generic correlations,
and a comparison of three initialization geometries.
...
| Family | Fixed model and data | Conclusion and scope |
|---|---|---|
...
| Part IV | \(m=2,L=2\), \(\phi=\arctan\), opposite labels, every fixed \(-1\le G_{12}<1\) | Strong compact containment for actual GF/raw-GD first-layer paths and velocities; no kinetic defect and convergence of residual-weighted first kernels along strong subsequences. No identified or unique population dynamics. |
| Part V | \(m=3\), fixed odd mixtures, convex offsets, and a calibrated depth-dependent activation | Sharp initialization conditioning orders, contraction/nonaffinity contrasts, and a sequential width-first/depth-second initialized correlation limit. No trained-flow result. |
...
Every training limit in Parts I–IV fixes the data, activation, finite depth
and physical horizon before \(n\to\infty\). Part V is initialization-only:
its width limit fixes depth and the entire activation, and its calibrated
family then takes a separate deterministic depth limit.
Each hidden layer has its own probability space;
...
Here \(c=1\) in Parts I, II and IV, and \(c=1/2\) in Part III. Consequently
\[
...
|---|---|---|---|
| I, II, IV | \(\sum_{a=1}^2r_a^2\) | \(\eta_n=n^{-2},\ t=k n^{-2}\) | Mean-loss step \(2n^{-2}\), mean-loss time \(2t\) |
...
## IV. Correlated opposite labels: strong first-layer compactness

This part supplies an actual-GF/raw-GD partial theorem at general fixed
correlations, not an all-layer population identification. It uses the shared
first-weight convention throughout. Its loss is the unhalved sum
\(\mathcal L_\Sigma=2\mathcal L_n\), so the corresponding mean-loss flow is
obtained by halving the velocity (or doubling physical time); a mean-loss GD
step is twice its sum-loss step for identical parameter updates. No optimizer
or initialization is changed by this bookkeeping.

### IV.1. Model, conventions, and theorem
...
\[
 |x_1|^2=|x_2|^2=d,\qquad
 X=[x_1,x_2],\qquad
 G=\frac{X^TX}{d}=\begin{pmatrix}1&\rho\\\rho&1\end{pmatrix},
 \qquad -1\le\rho<1.
 \tag{IV.1}
\]
...
For an array \(a=(a_i)_{i=1}^n\) of two-vectors, empirical averages
and space-time moments are written explicitly as
\[
 \frac1n\sum_{i=1}^n a_i,\qquad
 \frac{\|a\|_F}{\sqrt n}
   =\left(\frac1n\sum_{i=1}^n|a_i|^2\right)^{1/2},
 \qquad
 \left(\frac1n\sum_{i=1}^n\int_I|a_i(t)|^pdt\right)^{1/p}.
 \tag{IV.2}
\]
Here \(|a_i|\) is the ordinary norm of an individual two-vector,
whereas \(\|a\|_F\) is the ordinary Frobenius norm of the full array.

Both hidden activations are
\[
 \phi(s)=\arctan s,\qquad \phi'(s)=\frac1{1+s^2},
 \qquad B=\frac\pi2.
 \tag{IV.3}
\]
Thus \(|\phi|\le B\), \(0<\phi'\le1\), and
\(|\phi''(s)|=2|s|/(1+s^2)^2\le1\), using \(2|s|\le1+s^2\).
The readout is linear. Finite neuron fields use lowercase symbols
and hidden-layer superscripts; population coordinate fields introduced
below use capitals. Raw parameters use uppercase \(W^{(\ell)}\), with
full parameter tuple \(W=(W^{(1)},W^{(2)},W^{(3)})\).
Suppress the width index when no confusion is possible.
At width \(n\), let
\[
...
\[
\begin{aligned}
 z^{(1)}_a&=W^{(1)}x_a/\sqrt d,& h^{(1)}_a&=\phi(z^{(1)}_a),\\
 z^{(2)}_a&=W^{(2)}h^{(1)}_a,& h^{(2)}_a&=\phi(z^{(2)}_a),\\
 f_a&=\frac1n(W^{(3)})^Th^{(2)}_a,&r_a&=f_a-y_a,\\
 \delta^{(2)}_a&=W^{(3)}\odot \phi'(z^{(2)}_a),&
 q^{(1)}_a&=(W^{(2)})^T\delta^{(2)}_a,\\
 \delta^{(1)}_a&=\phi'(z^{(1)}_a)\odot q^{(1)}_a,&c_a&=-2r_a.
\end{aligned}
\tag{IV.4}
\]
...
\[
 \mathcal L_\Sigma(W)=r_1^2+r_2^2.
 \tag{IV.5}
\]
Gradient flow (GF) means the exact equations
\[
\begin{aligned}
 \dot W^{(1)}&=\frac1{\sqrt d}\sum_{a=1}^2c_a\delta^{(1)}_a x_a^T,\\
...
\[
\begin{aligned}
 W^{(1)}_{k+1}&=W^{(1)}_k+\frac\eta{\sqrt d}\sum_a
...
using (IV.4). In particular \(h^{(1)}(t)=\phi(z^{(1)}(t))\), and is generally
...
\[
 ...
 \tag{IV.8}
\]
For GD the derivatives in (IV.8) are taken almost everywhere along the raw
interpolation. Set
\[
...
\[
 ...
 \tag{IV.9}
\]
The \(L^2\) norms in (IV.9) use Lebesgue time measure without normalization.
For probability measures with finite second moment, \(\mathcal W_2\) is the
...
1. **Deterministic actual dynamics and compact containment.** Fix finite
   constants \(\alpha,\beta,b_4\ge0\), and impose only
   \[
    \|W^{(2)}_0\|_{\rm op}\le\alpha,\qquad
    \|W^{(3)}_0\|_\infty\le\beta,\qquad
    \frac1n\sum_i|z^{(1)}_i(0)|^4\le b_4.
    \tag{IV.10}
   \]
   ...
   in Sections IV.2–IV.5. There is a single deterministic compact set
   \[
    \mathcal K_{T,\rho;\alpha,\beta,b_4}
       \subset\mathcal P_2(\mathcal E_T)
    \tag{IV.11}
   \]
   containing every GF law satisfying (IV.10), at every width, and every
   GD law satisfying (IV.10), at every \(n\ge n_0\). This quantifies over
   **all** initial outcomes satisfying (IV.10), not one selected outcome
   ...
   antiparallel identities proved in Section IV.6.

2. **Actual Gaussian initialization.** Initialize all entries independently
   with
   \[
    W^{(1)}_{0,ij}\sim N(0,1),\quad
...
3. **Consequences along any \(\mathcal W_2\)-convergent subsequence.** For any
   deterministic selection of trajectories from the preceding families,
   and along any subsequence on which \(\mu_n\to\mu\) in \(\mathcal W_2\), let
   ...
   \begin{gathered}
    ...
    H^{(1)}=\phi(Z^{(1)}),\\
    S^{(1)}=\phi'(Z^{(1)})\odot V^{(1)}\quad\text{in }L^2,\qquad
   ...
   \[
    D=\begin{cases}G^{-1},&-1<\rho<1,\\ G/4,&\rho=-1,\end{cases}
    \qquad A_{\rm row}=XD/\sqrt d.
    \tag{IV.16}
   \]
   The empirical laws of the actual first-row increments and velocities
   converge in \(\mathcal W_2(C([0,T];\mathbb R^d)\times L^2([0,T];\mathbb R^d))\)
   to the law of
   \[
    ...
    \tag{IV.17}
   \]
   This convergence may also be taken jointly with the four fields in (IV.9).
   ...
   \begin{aligned}
    ...
    g_n^{(1)}(t):=\frac1n\|\dot W^{(1)}_n(t)\|_F^2&\longrightarrow
   ...
   \[
   \begin{aligned}
    k^{(1)}_{n,ab}(t)&=G_{ab}\frac1n\sum_i
                         \delta^{(1)}_{n,a,i}(t)\delta^{(1)}_{n,b,i}(t),\\
    j^{(1)}_{n,ab}(t)&=c_{n,a}(t)c_{n,b}(t)k^{(1)}_{n,ab}(t).
   \end{aligned}
   \tag{IV.19}
   \]
   Thus every finite symbol on the right of (IV.19) is barred in GD.
...
Section IV.12 states the precise limits of the conclusions.
...
give compact containment; fixed linear and quadratic maps give (IV.17)–(IV.21).

### IV.2. The raw metric and global finite gradient flow

For a raw tangent \(\xi=(\xi^{(1)},\xi^{(2)},\xi^{(3)})\), define
\[
 \|\xi\|_{\rm raw}^2=\frac1n\|\xi^{(1)}\|_F^2
                     +\|\xi^{(2)}\|_F^2
                     +\frac1n\|\xi^{(3)}\|^2.
 \tag{IV.22}
\]
Differentiating (IV.4) gives the ordinary Euclidean gradients
\[
 \nabla_{W^{(1)}}\mathcal L_\Sigma=\frac2{n\sqrt d}\sum_a r_a\delta^{(1)}_a x_a^T,
 \quad
 \nabla_{W^{(2)}}\mathcal L_\Sigma=\frac2n\sum_a r_a\delta^{(2)}_a(h^{(1)}_a)^T,
 \quad
 \nabla_{W^{(3)}}\mathcal L_\Sigma=\frac2n\sum_a r_a h^{(2)}_a.
 \tag{IV.23}
\]
Dividing these three blocks by the weights \(1/n,1,1/n\) in (IV.22)
shows that (IV.6) is \(-\operatorname{grad}_{\rm raw}\mathcal L_\Sigma\), and (IV.7) is
its exact Euler update in the raw parameters. Consequently, along GF,
\[
 \frac{d}{dt}\mathcal L_\Sigma=-\|\dot W\|_{\rm raw}^2
       =-\frac1n\|\dot W^{(1)}\|_F^2
...
and Lipschitz with a constant \(\Lambda\). On a sufficiently short interval the
...
At any putative finite terminal time \(t_*\), (IV.24) implies
\[
 \int_0^{t_*}\|\dot W\|_{\rm raw}^2dt\le\mathcal L_\Sigma(0),\qquad
 \|W(t)-W(s)\|_{\rm raw}\le\sqrt{|t-s|\mathcal L_\Sigma(0)}.
 \tag{IV.25}
\]
...
\tag{IV.26}
\]
Initially \(|f_a|\le B\beta\). By (IV.24), \(|r(t)|\le R_0\) and
...
nonzero. Applying this fact to the rank-one updates in (IV.6), using
\(|h^{(1)}_a|\le B\sqrt n\), gives \(A_{\rm F}\) in (IV.26).
...
\(\dot z^{(1)}_a=\sum_b G_{ab}c_b\delta^{(1)}_b\) and
\(|G_{ab}|\le1\). If the last bound in (IV.10) also holds, Minkowski and
Cauchy–Schwarz give the explicit first-primal bound
\[
 \frac{\|z^{(1)}_a(t)\|}{\sqrt n}
       \le b_4^{1/4}+T K_c^{\rm F}Q_{\rm F}.
 \tag{IV.28}
\]
...
\begin{aligned}
 \dot z^{(2)}_a&=\dot W^{(2)}h^{(1)}_a+W^{(2)}\dot h^{(1)}_a,\\
 \dot\delta^{(2)}_a
   &=\dot W^{(3)}\odot \phi'(z^{(2)}_a)
            +W^{(3)}\odot \phi''(z^{(2)}_a)\odot\dot z^{(2)}_a,\\
...
\[
 k_{ab}=G_{ab}\frac{(\delta^{(1)}_a)^T\delta^{(1)}_b}{n}
       +\frac{(\delta^{(2)}_a)^T\delta^{(2)}_b}{n}
                           \frac{(h^{(1)}_a)^Th^{(1)}_b}{n}
       +\frac{(h^{(2)}_a)^Th^{(2)}_b}{n}.
 \tag{IV.32}
\]
Each term is obtained by inserting its corresponding equation in (IV.6)
...
### IV.3. GF row work and the gain in moments

Define the controlled query and its gated version by
\[
 u^{(1)}_{a,i}=c_aq^{(1)}_{a,i},\qquad
 e^{(1)}_{a,i}=\phi'(z^{(1)}_{a,i})u^{(1)}_{a,i}
                         =c_a\delta^{(1)}_{a,i}.
 \tag{IV.34}
\]
Use the GF constants from (IV.26)–(IV.33) and put
\[
 ...
 \tag{IV.35}
\]
In (IV.35), the scalar \(V_{\rm F}\) is an envelope constant, distinct from
the population velocity \(V^{(1)}\). The product rule and (IV.31),(IV.33) give
\[
 \left(\frac1n\sum_i
...
\(X\). Equations (IV.6) and (IV.34) give
\[
 \dot W^{(1)}_i=Xe^{(1)}_i/\sqrt d,\qquad
 v^{(1)}_i=Ge^{(1)}_i,\qquad
 |\dot W^{(1)}_i|^2=(e^{(1)}_i)^TGe^{(1)}_i
                         =\sum_a u^{(1)}_{a,i}s^{(1)}_{a,i}.
 \tag{IV.39}
\]
The last equality uses \(s^{(1)}_{a,i}=\phi'(z^{(1)}_{a,i})v^{(1)}_{a,i}\).
The left side is nonnegative for every correlation in (IV.1).
Integration by parts, including both boundary terms, proves that the
one-row raw action
\[
 \mathcal A_i^{\rm F}:=\int_0^T |\dot W^{(1)}_i|^2dt
\]
satisfies
\[
\begin{aligned}
...
The eigenvalues of \(G\) are \(1+\rho\) and \(1-\rho\), in \([0,2]\).
Diagonalizing this explicit symmetric matrix gives
\(|Ge|^2\le2e^TGe\). Also \(|e^{(1)}_i|\le\upsilon_i^{(1)}\). Hence
\[
 |v^{(1)}_i|^2\le2|\dot W^{(1)}_i|^2,\quad
...
\]
Using \((a+b)^4\le8(a^4+b^4)\) and (IV.40), we obtain the actual GF bounds
\[
\begin{aligned}
 \frac1n\sum_i\|z^{(1)}_i\|_\infty^4
       &\le8b_4+128B^2T^2V_{\rm F}^2,\\
...
\tag{IV.42}
\]
All these calculations remain valid for singular \(G\); no inverse has
yet been used.

### IV.4. Exact raw GD: a closed descent and primal estimate
...
\begin{aligned}
 D_\zeta D_\xi z^{(2)}_a
   ={}&\xi^{(2)}[\phi'(z^{(1)}_a)\odot(\zeta^{(1)}x_a/\sqrt d)]
       +\zeta^{(2)}[\phi'(z^{(1)}_a)\odot(\xi^{(1)}x_a/\sqrt d)]\\
      &+W^{(2)}[\phi''(z^{(1)}_a)\odot(\zeta^{(1)}x_a/\sqrt d)
                                      \odot(\xi^{(1)}x_a/\sqrt d)].
\end{aligned}
\tag{IV.45}
\]
The first two terms have ordinary Euclidean norm divided by
\(\sqrt n\) at most one each. For the last use
\[
...
\((\sum_i u_i^2)(\sum_jv_j^2)\). Thus the ordinary Euclidean norm of (IV.45), divided by
\(\sqrt n\), is at most \(2+A\sqrt n\). The full second prediction derivative consists of
\[
\begin{aligned}
 D_\zeta D_\xi f_a=\frac1n\bigl(&
  (\xi^{(3)})^T[\phi'(z^{(2)}_a)\odot D_\zeta z^{(2)}_a]
 +(\zeta^{(3)})^T[\phi'(z^{(2)}_a)\odot D_\xi z^{(2)}_a]\\
 &+(W^{(3)})^T[\phi''(z^{(2)}_a)\odot D_\zeta z^{(2)}_a
                                           \odot D_\xi z^{(2)}_a]
 +(W^{(3)})^T[\phi'(z^{(2)}_a)\odot D_\zeta D_\xi z^{(2)}_a]\bigr).
\end{aligned}
\]
...
\(K_cF_*\), by (IV.23). Integrating the first prediction differential
...
\[
 D^2\mathcal L_\Sigma=2\sum_a(Df_a\otimes Df_a+r_aD^2f_a)
\]
and (IV.46) bound the raw Hessian norm there by
\[
 H_*(n)=4F_*^2+2\sqrt2(R+1)F_{**}(n).
 \tag{IV.47}
\]
For \(g_k=\operatorname{grad}_{\rm raw}\mathcal L_\Sigma(W_k)\), the scalar integral
Taylor formula along \(W_k-tg_k\), \(0\le t\le\eta\), now proves
\[
 \mathcal L_{\Sigma,k+1}\le\mathcal L_{\Sigma,k}-\eta\|g_k\|_{\rm raw}^2
                  +\frac{\eta^2H_*(n)}2\|g_k\|_{\rm raw}^2
       \le\mathcal L_{\Sigma,k}-\frac\eta2\|g_k\|_{\rm raw}^2
 \tag{IV.48}
\]
...
Induction in (IV.48) keeps the candidate exit node at
\(|r_{k+1}|\le R_0<R\), contradicting exit. Hence no exit occurs.

This proves actual GD stability through \(N\), including
\[
 \mathcal L_{\Sigma,k}\le\mathcal L_{\Sigma,0},\qquad
 \frac12\sum_{k=0}^{N-1}\eta
       \left[\frac1n\left\|\frac{W^{(1)}_{k+1}-W^{(1)}_k}{\eta}\right\|_F^2
        +\left\|\frac{W^{(2)}_{k+1}-W^{(2)}_k}{\eta}\right\|_F^2
        +\frac1n\left\|\frac{W^{(3)}_{k+1}-W^{(3)}_k}{\eta}\right\|^2
       \right]\le\mathcal L_{\Sigma,0}.
 \tag{IV.49}
\]
Node loss decrease is not an exact loss-dissipation identity inside a
raw affine cell.

### IV.5. Actual GD query regularity and discrete row work

Continue with the constants in (IV.43) and use (IV.29) with those constants.
...
\[
 ...
 \tag{IV.50}
\]
For the **recomputed** nonlinear fields, the product rules (IV.30) hold
...
\begin{aligned}
 ...
 \Delta\delta^{(2)}_a&=\Delta W^{(3)}\odot \phi'(z^{(2)}_{k,a})
             +W^{(3)}_{k+1}\odot[\phi'(z^{(2)}_{k+1,a})-\phi'(z^{(2)}_{k,a})],\\
 \Delta q^{(1)}_a&=(\Delta W^{(2)})^T\delta^{(2)}_{k,a}
                            +(W^{(2)}_{k+1})^T\Delta\delta^{(2)}_a.
\end{aligned}
\tag{IV.52}
\]
They bound \(\|\Delta z^{(2)}_a\|/\sqrt n\),
\(\|\Delta\delta^{(2)}_a\|/\sqrt n\), and
\(\|\Delta q^{(1)}_a\|/\sqrt n\) by
\(\eta D_Z\), \(\eta D_\delta\), and \(\eta D_q\), respectively, while
\(\sum_a|\Delta c_a|\le\eta D_c^{\rm G}\). The first preactivation
also satisfies
\[
 \frac{\|z^{(1)}_a(t)\|}{\sqrt n}
     \le b_4^{1/4}+H K_c^{\rm G}Q_{\rm G}.
 \tag{IV.53}
\]
The remaining actual primal bounds are exactly the nondifferential
inequalities (IV.27) with the constants in (IV.43); along segments the residual
...
Both endpoint controls are bounded after (IV.48), so
\[
...
At node \(k\) put \(e^{(1)}_{k,a,i}=\phi'(z^{(1)}_{k,a,i})u^{(1)}_{k,a,i}\)
and \(a_{k,i}=(e^{(1)}_{k,i})^TGe^{(1)}_{k,i}\). The exact raw update gives
\[
 \left|\frac{W^{(1)}_{k+1,i}-W^{(1)}_{k,i}}\eta\right|^2=a_{k,i},
 \qquad \Delta z^{(1)}_i=\eta Ge^{(1)}_{k,i},\qquad
...
\(\eta\max_i\upsilon_i^{(1)}\le V_{\rm G}n^{-3/2}\le1/2\). This uses (IV.56),
...
\end{aligned}
\]
Its absolute value is at most \(2B\upsilon_i^{(1)}\). Summing (IV.58) and retaining
...
and are independent of \(b_4,d,\rho,n\); the displayed function \(H_*(n)\)
has the growth stated in (IV.47).
Thus such an integer exists, without any stability assumption.

On the raw interpolation \(v^{(1)}_i=Ge^{(1)}_{k,i}\) on each cell.
Equations (IV.56)–(IV.59) give, also on a partial terminal cell,
\[
 ...
 \tag{IV.61}
\]
Furthermore \(s^{(1)}_i=\phi'(z^{(1)}_i)\odot v^{(1)}_i\), with the
...
\[
\begin{aligned}
 \frac1n\sum_i\|z^{(1)}_i\|_\infty^4&\le8b_4+512B^2H^2V_{\rm G}^2,\\
...
### IV.6. Exact antiparallel identities with the actual readout

At \(\rho=-1\), (IV.1) gives \(|x_1+x_2|^2=0\), so \(x_2=-x_1\).
...
\[
 ...
 \tag{IV.63}
\]
Since \(\phi'\) is even, (IV.4) then gives
\[
...
preserves the first equality in (IV.64). With the actual opposite labels,
\[
 r_2=-f_1+1=-r_1,\qquad c_2=-c_1.
 \tag{IV.65}
\]
There is no assertion that \(f_1(0)=0\), or that the readout is zero.
Substituting (IV.63)–(IV.65) into the full GF equations gives
\[
 \dot W^{(1)}=\frac{2c_1}{\sqrt d}\delta^{(1)}_1x_1^T,\qquad
...
cell, again satisfies (IV.63)–(IV.65), because those identities hold for every
parameter choice. Thus this is an exact architecture-enforced relation,
including the actual nonzero Gaussian readout (IV.12).
...
The matrix \(G\) acts as multiplication by two on \(E_-\), so
\[
 v^{(1)}_i=2e^{(1)}_i,\qquad e^{(1)}_i=Dv^{(1)}_i,
 \qquad D=G/4,\qquad |e^{(1)}_i|=|v^{(1)}_i|/2.
 \tag{IV.67}
\]
...
### IV.7. Strong velocity translations, uniformly over all GD widths
...
\begin{aligned}
 ...
 B_{\rm path}&=8b_4+\max\{128B^2T^2V_{\rm F}^2,
                                      512B^2H^2V_{\rm G}^2\}.
\end{aligned}
\tag{IV.70}
\]
...
\[
\begin{gathered}
 \left(\frac1n\sum_{i=1}^n|v^{(1)}_i(t)|^2\right)^{1/2}\le K_z,\\
 \left(\frac1n\sum_{i=1}^n\int_0^T|v^{(1)}_i(t)|^3dt\right)^{1/3}\le J,\\
...
\(\|h^{(1)}_i\|_\infty\le\sqrt2 B\). Equations (IV.36),(IV.54) give the
...
\[
\begin{aligned}
 \left(\frac1n\sum_{i=1}^n\int_{I_\tau}
       |u^{(1)}_i(t+\tau)-u^{(1)}_i(t)|^2dt\right)^{1/2}
                     &\le\sqrt T K_u(\tau+\epsilon),\\
 \left(\frac1n\sum_{i=1}^n\int_{I_\tau}
       |z^{(1)}_{{\rm gate},i}(t+\tau)-z^{(1)}_{{\rm gate},i}(t)|^2dt\right)^{1/2}
                     &\le\sqrt T K_z(\tau+\epsilon),\\
 \left(\frac1n\sum_{i=1}^n\int_{I_\tau}
       |z^{(1)}_i(t+\tau)-z^{(1)}_i(t)|^2dt\right)^{1/2}
                     &\le\sqrt T K_z\tau.
\end{aligned}
\tag{IV.72}
\]
For the second GD bound, each first-node difference is \(\eta\) times
the corresponding cell velocity, so
\(\left(\frac1n\sum_{i=1}^n|\Delta z^{(1)}_i|^2\right)^{1/2}
\le\eta K_z\). The last line integrates the actual affine velocity.

We derive the special arctan gate estimate. The logarithmic derivative
\((\log\phi')'(x)=-2x/(1+x^2)\) has absolute value at most one. For positive
numbers \(a,b\), division by their geometric mean gives
\(|a-b|/(a+b)=\tanh(|\log a-\log b|/2)\). Since
\(0\le\tanh t\le\min(1,t)\) for \(t\ge0\),
\[
 \theta(x,y):=\frac{|\phi'(x)-\phi'(y)|}{\phi'(x)+\phi'(y)}
                \le\min(1,|x-y|/2).
 \tag{IV.73}
\]
If \(a=\phi'(x)u\), \(b=\phi'(y)v\), then
\[
\begin{aligned}
 |a-b|&\le \phi'(x)|u-v|+\theta(\phi'(x)+\phi'(y))|v|\\
       &\le \phi'(x)(1+\theta)|u-v|+\theta(|a|+|b|)\\
       &\le2|u-v|+\theta(|a|+|b|).
\end{aligned}
\tag{IV.74}
\]
Here the second line uses \(\phi'(x)|v|\le|a|+\phi'(x)|u-v|\). Thus (IV.74)
...
Apply (IV.74) at \(t,t+\tau\) to
\(e^{(1)}_i=\phi'(z^{(1)}_{{\rm gate},i})\odot u^{(1)}_i\).
By (IV.67)–(IV.71),
\(\left(\frac1n\sum_{i=1}^n\int_0^T|e^{(1)}_i(t)|^3dt\right)^{1/3}\le\kappa_\rho J\).
The maximum gate ratio \(\theta_i(t)\) lies in \([0,1]\), and (IV.72),(IV.73)
give
\[
 \frac1n\sum_{i=1}^n\int_{I_\tau}|\theta_i(t)|^6dt
    \le\frac1n\sum_{i=1}^n\int_{I_\tau}|\theta_i(t)|^2dt
...
\begin{aligned}
 &\left(\frac1n\sum_{i=1}^n\int_{I_\tau}
       |e^{(1)}_i(t+\tau)-e^{(1)}_i(t)|^2dt\right)^{1/2}\\
 &\qquad\le2\sqrt T K_u(\tau+\epsilon)
       +2\kappa_\rho J(TK_z^2/4)^{1/6}(\tau+\epsilon)^{1/3}.
\end{aligned}
 \tag{IV.75}
\]
Multiplication by \(G\), whose norm is at most two, gives the same
...
constant \(C_0\), depending only on the fixed constants in (IV.70) and on
\(\rho,T\), such that
\[
 \frac1n\sum_{i=1}^n\int_{I_\tau}
       |v^{(1)}_i(t+\tau)-v^{(1)}_i(t)|^2dt
...
\begin{aligned}
 s^{(1)}(t+\tau)-s^{(1)}(t)
   ={}&\phi'(z^{(1)}(t+\tau))\odot[v^{(1)}(t+\tau)-v^{(1)}(t)]\\
     &+[\phi'(z^{(1)}(t+\tau))-\phi'(z^{(1)}(t))]\odot v^{(1)}(t).
\end{aligned}
\]
The largest coordinate gate difference is at most
\(\min(1,|z^{(1)}_i(t+\tau)-z^{(1)}_i(t)|)\), since \(\phi'\in(0,1]\)
and \(|\phi''|\le1\). By (IV.72),
\[
 \left(\frac1n\sum_{i=1}^n\int_{I_\tau}
    \max_{a=1,2}|\phi'(z^{(1)}_{a,i}(t+\tau))
                  -\phi'(z^{(1)}_{a,i}(t))|^6dt\right)^{1/6}
       \le(TK_z^2)^{1/6}\tau^{1/3}.
\]
Another application of Hölder proves
\[
\begin{aligned}
 &\left(\frac1n\sum_{i=1}^n\int_{I_\tau}
       |s^{(1)}_i(t+\tau)-s^{(1)}_i(t)|^2dt\right)^{1/2}\\
 &\qquad\le\left(\frac1n\sum_{i=1}^n\int_{I_\tau}
       |v^{(1)}_i(t+\tau)-v^{(1)}_i(t)|^2dt\right)^{1/2}
                          +J(TK_z^2)^{1/6}\tau^{1/3}.
\end{aligned}
 \tag{IV.77}
\]
...
The \(\eta\) in (IV.76) must be removed uniformly to prove (IV.11) for all
...
(IV.76) gives, for GD, the useful minimum bound
\[
 \frac1n\sum_{i=1}^n\int_{I_\tau}
       |v^{(1)}_i(t+\tau)-v^{(1)}_i(t)|^2dt
...
If \(\eta\le\delta\), the first term in (IV.79) is at most
...
\[
\begin{aligned}
 \frac1n\sum_{i=1}^n\int_{I_\tau}\bigl(
       &|v^{(1)}_i(t+\tau)-v^{(1)}_i(t)|^2\\
       &+|s^{(1)}_i(t+\tau)-s^{(1)}_i(t)|^2\bigr)dt
                         \le C_1\tau^{2/5}.
\end{aligned}
 \tag{IV.80}
\]
For GF, (IV.76) has \(\epsilon=0\), and implies this weaker exponent too.
Equation (IV.77) supplies the activation term because
\(\tau^{2/3}\le\tau^{2/5}\) when \(\tau\le1\).
Thus (IV.80) is uniform over every outcome satisfying (IV.10), every GF width,
and every GD width \(n\ge n_0\). The exponent \(2/5\) bounds the
**squared** space-time \(L^2\) shift norm. No optimal exponent is claimed.

### IV.8. Elementary strong joint compactness and tightness
...
over neurons and using (IV.80) proves
\[
...
own projection. Equations (IV.82),(IV.83) yield the uniform bound
\[
 \mathcal W_2(\mu_n,\mathcal Q_{h\#}\mu_n)^2
...
The complete tuple has a uniform fourth moment. Indeed, by (IV.71),
\[
...
zero. By (IV.85) the squared transport cost is at most \(M_4/R^2\).
...
This proves total boundedness of the projected laws in \(\mathcal W_2\).
Together with (IV.84), it proves total boundedness of the union of **all**
...
product norm in (IV.9). Thus \(\xi_j\) has a measurable limit \(\xi\);
...
\]
The limit has finite second moment and its law is the required \(\mathcal W_2\)
...
then gives a finite subcover. This proves (IV.11).
...
\[
 \mathcal G=\{\xi:\|\xi\|_{\mathcal E_T}\le R,
...
makes \(\mathcal G\) compact. Markov's inequality and the union bound give
\[
 \mu_n(\mathcal G^c)\le M_4/R^4+
          \sum_{j\ge1}2^{2j}\varepsilon(h_j)\le a.
\]
This is uniform tightness of the same-neuron tuple laws themselves.
Together with (IV.85) it also explicitly controls their quadratic tails.
...
### IV.9. Gaussian good events and probability quantifiers

Under (IV.12), different first rows give independent Gaussian pairs with
covariance \(G\). Write one such pair as
\((U,\rho U+\sqrt{1-\rho^2}U')\) for independent standard normals
\(U,U'\); this formula remains valid at \(\rho=-1\).
Gaussian integration by parts gives
\(\mathbb E U^{2k}=(2k-1)\mathbb E U^{2k-2}\), starting from one:
differentiate \(e^{-x^2/2}\), integrate \(x^{2k-1}\) against its
derivative, and use its vanishing boundary terms. Thus
\(\mathbb E U^4=3\), \(\mathbb E U^8=105\). Expanding the above
representation gives
\(\mathbb E[U^2(\rho U+\sqrt{1-\rho^2}U')^2]=1+2\rho^2\).
Therefore
\[
...
\[
 ...
 \tag{IV.90}
\]
No independence between the three events is needed to add (IV.88)–(IV.90).
This proves (IV.13). Take \((\alpha,\beta,b_4)=(8,1,13)\) in the deterministic
construction (IV.11). Every point of \(E_n\) belongs to its hypotheses,
for both schemes at the same initialization, proving (IV.14) and the stated
joint probability bounds.

The random empirical laws used in (IV.14) are measurable. At fixed width,
...
given initial point, (IV.25) bounds the finite-dimensional solutions through
\(T\) in a common ball. The smooth vector field is Lipschitz there with a
constant \(\Lambda\).
Iteration of the difference-integral inequality bounds the difference
of solutions by their initial difference times
\(\sum_{k\ge0}(\Lambda T)^k/k!=e^{\Lambda T}\); evaluating the vector field also
...
turns these statements into continuity of \(\mu_n\) in \(\mathcal W_2\).
...
\]
On \(E_n\), (IV.85) and the cubic bound give
...
The bound (IV.13) is uniform over deterministic normalized input pairs and
...
realization is not asserted. Nor does (IV.13), whose leading term is not
summable over all widths, assert eventual goodness almost surely along
the whole width sequence.

### IV.10. Population compatibility and raw first-row reconstruction

Fix any deterministic subsequence for which \(\mu_n\to\mu\) in
\(\mathcal W_2(\mathcal E_T)\). All assertions in this and the next section are
...
\(z(t)-z(0)-\int_0^t v(u)du\) is continuous from
\(C([0,T];\mathbb R^2)\times L^2([0,T];\mathbb R^2)\)
to \(C([0,T];\mathbb R^2)\), because its difference is bounded in the uniform norm by
...
\[
 \|\phi'(z_j)\odot v_j-\phi'(z)\odot v\|_2
     \le\|v_j-v\|_2+\|\phi'(z_j)-\phi'(z)\|_\infty\|v\|_2\longrightarrow0.
 \tag{IV.92}
\]
Thus the set of tuples obeying all of (IV.15) is closed. Every empirical
...
\(\varepsilon_n\), and is therefore zero. This proves (IV.15).
...
For each finite row, (IV.39) holds in GF and (IV.57) gives its GD cell version.
With the held node \(e^{(1)}\) in GD, (IV.67),(IV.68) therefore prove
\[
 e^{(1)}_i=Dv^{(1)}_i,\qquad
 \dot W^{(1)}_i=A_{\rm row}v^{(1)}_i,\qquad
 |\dot W^{(1)}_i|^2=(v^{(1)}_i)^TDv^{(1)}_i.
 \tag{IV.93}
\]
Indeed \(DGD=D\) in both cases, so
\[
 A_{\rm row}^TA_{\rm row}=DGD=D,\qquad
 \|A_{\rm row}\|_{\rm op}^2=\kappa_\rho.
 \tag{IV.94}
\]
At the antiparallel endpoint, write \(e^{(1)}_i=(b,-b)\). Then
\(v^{(1)}_i=(2b,-2b)\),
\(\dot W^{(1)}_i=2bx_1/\sqrt d\), and
\[
 |\dot W^{(1)}_i|^2=4b^2
...
The exact invariance in Section IV.6 is essential for reconstructing
\(e^{(1)}\) at this endpoint. Without it a component in the nullspace
of \(G\) would be invisible to \(v^{(1)}\) but could change individual
controlled kernel entries.

Integrating (IV.93) and using (IV.8) proves
\[
 W^{(1)}_i(t)-W^{(1)}_i(0)
       =A_{\rm row}[z^{(1)}_i(t)-z^{(1)}_i(0)].
 \tag{IV.96}
\]
The map from \((z,v,h,s)\) to the two right-hand sides of (IV.96),(IV.93)
...
tuple as extra coordinates adds just its original cost. This proves (IV.17),
...
not reconstruct full rows, put \(P=XD X^T/d\). Symmetry and \(DGD=D\)
...
### IV.11. No kinetic defect and every node-controlled kernel entry
...
In the coupling of Section IV.10 put
\[
...
of (IV.18). For the raw row form, for any two vectors \(v,w\),
\[
 |v^TDv-w^TDw|\le\|D\|_{\rm op}|v-w|(|v|+|w|).
\]
By (IV.93) and the same coupling estimate,
\[
 \|g_n^{(1)}-G^{(1)}\|_{L^1}
        \le\kappa_\rho\varepsilon_n(M_n+M)\longrightarrow0.
 \tag{IV.100}
\]
This proves all of (IV.18). For a measurable \(A\subset[0,T]\), the
...
\[
 \lim_n\frac1n\int_A\|\dot W^{(1)}_n(t)\|_F^2dt
       =\int_A\mathbb E_\mu[(V^{(1)}(t))^TDV^{(1)}(t)]dt.
 \tag{IV.101}
\]
The ordinary unnormalized matrix speed is \(n\) times \(g_n^{(1)}\).
...
they can be placed inside the finite sum in (IV.19). With the node
convention in GD, (IV.93) gives the exact identity
\[
 j^{(1)}_{n,ab}=G_{ab}\frac1n\sum_i
...
\[
 \mathcal H(v)(t)=G\odot[(Dv(t))(Dv(t))^T].
 \tag{IV.103}
\]
For vectors \(e,f\),
\(ee^T-ff^T=(e-f)e^T+f(e-f)^T\), so
\(\|ee^T-ff^T\|_F\le|e-f|(|e|+|f|)\).
Entrywise multiplication by \(G\) does not increase the Frobenius norm,
because every \(|G_{ab}|\le1\). Integrating in time therefore proves
\[
\begin{aligned}
...
\tag{IV.104}
\]
Here \(\|A\|_{L^1(F)}=\int_0^T\|A(t)\|_Fdt\). Equation (IV.98) and
...
formula (IV.20). Taking its difference from (IV.102) inside the coupling and
using (IV.104) gives the full-matrix estimate
\[
 \|j_n^{(1)}-J^{(1)}\|_{L^1(F)}
...
to the exact dissipation (IV.24). In a GD cell, if
\(g_k=\operatorname{grad}_{\rm raw}\mathcal L_\Sigma(W_k)\), the actual loss derivative
is instead
\[
 \frac d{dt}\mathcal L_\Sigma(W(t))
        =-\langle\operatorname{grad}_{\rm raw}\mathcal L_\Sigma(W(t)),g_k\rangle_{\rm raw}.
 \tag{IV.106}
\]
It need not equal \(-\|g_k\|_{\rm raw}^2\). Equations (IV.102),(IV.105)
...
in \(\mathcal W_2\), by the coupling bounds in (IV.94),(IV.99),(IV.100),(IV.105). Take the
joint image of the compact set (IV.11) under these maps, retaining the
...
\(\mathcal W_2\) spaces, three scalar \(L^1\) spaces, and matrix-valued \(L^1\).
It contains all the actual good-outcome observables, so their joint
failure probability has the same bound (IV.13). This conclusion supplies
controlled-kernel compactness without requiring convergence of residuals
or of the unweighted kernel as separate objects.

### IV.12. Exact scope of the result

The compact set (IV.11) contains all good deterministic outcomes, not just
...
interleaved, has a \(\mathcal W_2\)-convergent subsequence. Along **any** actual
\(\mathcal W_2\)-convergent subsequence, (IV.15)–(IV.21) hold for its own limiting law.
For the original random laws, (IV.14) is compact containment in probability.
...
- Bounds on expectations over external initialization randomness on bad
  ...
  (IV.13),(IV.14),(IV.91) do not supply those assertions.
...
the initial orthogonal component in (IV.97). None of these additional
...
There is also a positive fraction of derivative-active rows: under any raw GF satisfying the locally integrable row-coefficient condition stated below, a row initially outside \(P\) never enters \(P\) in finite time. Consequently the population fraction of rows with at least one strictly positive first-layer activation derivative remains at least
...
It also gives relative compactness here. Select a Cauchy subsequence of
laws whose consecutive distances are at most \(2^{-j}\), and approximate
each by a finitely supported law within \(2^{-j}\), using the construction
above. The consecutive distances of the latter laws are summable. Choose
finite couplings with summable root mean squared costs and join them into
a chain: condition each next finite variable on the preceding value using
the coupling probabilities, with arbitrary choices on zero-mass values.
The triangle inequality in \(L^2\) shows that the sum of the path distances
has finite second moment. Thus the chain is almost surely Cauchy in the
uniform path metric. Continuous paths form a complete space (uniform limits
are continuous), so the chain converges to a continuous random path. Its
\(L^2\) distance from the chain tends to zero, bounded by the summable tail
of the successive \(L^2\) distances. Its law has finite second moment.
The triangle inequality transfers this \(\mathcal W_2\) limit back to the
selected original subsequence. This proves the compactness conclusion
without asserting that its limiting law solves a field equation.
...
Raw second preactivations, incoming fields \(Q_a\), and the uncapped physical first kernel
...
The full-limit proofs in Parts I–III retain actual Gaussian initialization, the trained operator
...
Part IV is a distinct partial result for actual correlated-data dynamics.
Its strong first-layer topology retains kinetic energy and the complete
residual-weighted first-kernel matrix along converging subsequences, including
off-diagonal entries. It proves neither a common GF/GD subsequential limit nor
an autonomous population equation, full-sequence convergence, or unweighted
kernel convergence. Its two endpoint conventions and raw-row reconstruction
are stated in that part; no global-limit conclusion is inferred from compactness.

Part V concerns initialized geometry only. Absolute and variance-normalized
sample conditioning, and scalar affine-regression residuals, are different
quantities. Its calibrated depth limit is sequential after width, with a
depth-dependent activation; none of its Gaussian recursions is asserted for
trained non-Gaussian fields. These comparisons neither extend Parts I–III nor
turn their absolute nonaffinity into a depth-uniform relative margin.
